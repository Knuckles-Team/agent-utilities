...
async def main():
    ...
    for q, label in [
        ...
        ("MATCH (n) WHERE n.embedding IS NOT NULL RETURN count(n) AS c", "w/ embedding"),
        ("MATCH (n) WHERE n.text IS NOT NULL RETURN count(n) AS c", "w/ text"),
    ]:
        t = time.monotonic()
        try:
            print(f"  {label}: {b.execute(q)}  [{time.monotonic()-t:.2f}s]", flush=True)
        except Exception as e:
            print(f"  {label}: ERR {type(e).__name__}: {str(e)[:200]}  [{time.monotonic()-t:.2f}s]", flush=True)

    print("\n=== A2. SEMANTIC SEARCH quality ===", flush=True)
    for text in ["List recent ServiceNow incidents.", "servicenow incident", "knowledge graph engine"]:
        t = time.monotonic()
        try:
            ...
            print(f"  {text!r} ERR {type(e).__name__}: {str(e)[:250]} [{time.monotonic()-t:.2f}s]", flush=True)
    ...
    if cli is None:
        ...
        for opname, fn in [
            ...
            ("health", lambda: cli.health()) if hasattr(cli, "health") else ("skip", None),
        ]:
            if fn is None:
                continue
            lat = []
            for _ in range(60):
                ...
                try:
                    ...
                    print(f"  {opname} ERR {type(e).__name__}: {str(e)[:120]}", flush=True)
                    break
            ...
            print(f"  {opname}: n={len(lat)} min={lat_s[0]*1000:.2f}ms "
                  f"p50={lat_s[len(lat_s)//2]*1000:.2f}ms p90={lat_s[int(len(lat_s)*.9)]*1000:.2f}ms "
                  f"max={lat_s[-1]*1000:.2f}ms", flush=True)
            print(f"    FAST(<50ms): {len(fast)} calls, mean {statistics.mean(fast)*1000:.2f}ms" if fast else "    FAST: none", flush=True)
