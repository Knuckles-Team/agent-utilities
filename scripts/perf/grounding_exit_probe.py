...
async def main():
    ...
    for ep, br in eb._breakers.items():
        print(
            f"  {ep!r}: state={br._state} failures={br._failures} threshold={br.threshold} cooldown={br.cooldown} enabled={br.enabled}",
            flush=True,
        )
    if not eb._breakers:
        print("  (no breakers constructed yet in this process)", flush=True)

    from agent_utilities.core.config import config

    print(
        f"  config: threshold={config.engine_breaker_threshold} cooldown={config.engine_breaker_cooldown}",
        flush=True,
    )
    import os

    for k in (
        "GRAPH_SERVICE_RPC_TIMEOUT",
        "GRAPH_SERVICE_CONNECT_TIMEOUT",
        "GRAPH_SERVICE_HEAVY_RPC_TIMEOUT",
        "GRAPH_SERVICE_WRITE_TIMEOUT",
    ...
    print(
        f"\n=== bounded compile (_CONTEXT_COMPILE_TIMEOUT_S={cm._CONTEXT_COMPILE_TIMEOUT_S}) x5 ===",
        flush=True,
    )
    ...
    for i in range(5):
        ...
        try:
            if asyncio.iscoroutinefunction(fn):
                await fn(msgs, "qwen/qwen3.8-27b")
            else:
                await asyncio.to_thread(fn, msgs, "qwen/qwen3.8-27b")
            print(f"  [{i}] OK in {time.monotonic() - t:.2f}s", flush=True)
        except Exception as e:
            print(
                f"  [{i}] RAISED after {time.monotonic() - t:.2f}s:\n  {chain(e)}",
                flush=True,
            )
