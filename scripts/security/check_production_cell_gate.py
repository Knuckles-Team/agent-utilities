#!/usr/bin/env python3
"""Fast-tier forwarder: production cell source contract (D-ML-1).

Delegates, unmodified, to ``scripts/deployment/check_production_assets.py``
— dark since 2026-07-28 because it was wired only into the push-triggered
``.github/workflows/guardrails.yml`` ("Production cell source contract"
...
Usage:
  python3 scripts/security/check_production_cell_gate.py
  python3 scripts/security/check_production_cell_gate.py --repository-root DIR
  python3 scripts/security/check_production_cell_gate.py --self-check
...
TARGET = "scripts/deployment/check_production_assets.py"
EXTRA_ARGS: list[str] = []


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="check-production-cell-gate")
