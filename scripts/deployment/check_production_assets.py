...
def _engine_identity_contract(directory: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise ProductionAssetError(
            "canonical engine identity contract is absent or malformed"
        ) from exc
    if not isinstance(contract, dict):
        raise ProductionAssetError(
            "canonical engine identity contract must be an object"
        )
    if contract.get("authority") != _ENGINE_IDENTITY_REF:
        raise ProductionAssetError(
            "engine identity contract authority is not canonical"
        )
    if (
        contract.get("$schema")
        != "https://agent-utilities.invalid/schemas/engine-identity-contract-v1.json"
        or contract.get("apiVersion") != "agent-utilities.io/engine-identity/v1"
        or contract.get("kind") != "EngineIdentityContract"
        or contract.get("contract_version") != "engine-identity.v1"
    ):
        raise ProductionAssetError(
            "engine identity contract version or schema is not canonical"
        )
    ...
    if digest != expected:
        raise ProductionAssetError(
            "engine identity contract digest does not match canonical content"
        )
    if contract.get("live_authority") is True:
        raise ProductionAssetError(
            "engine identity contract cannot opt into an unbounded live authority"
        )
    expected = {
    ...
    if not aliases or any(
        ...
        or alias.get("status") != "migration-only"
        or alias.get("live_authority") is not False
        for alias in aliases
    ):
        raise ProductionAssetError(
            "engine identity compatibility aliases must remain migration-only"
        )
    return contract
...
def _validate_config(
    ...
    *,
    ...
    expected_cell = {
        "EPISTEMIC_GRAPH_RAFT_GROUPS": str(
            topology_data.get("ENGINE_RAFT_GROUPS", "20")
        ),
    ...
    if identity is not None:
        if cell.get("ENGINE_IDENTITY_CONTRACT_REF") != _ENGINE_IDENTITY_REF or cell.get(
            "ENGINE_IDENTITY_CONTRACT_DIGEST"
        ) != identity.get("digest"):
            raise ProductionAssetError(
                "cell contract does not bind the canonical engine identity contract"
            )


def _validate_engine(
    documents: list[dict[str, Any]], *, identity: dict[str, Any]
) -> None:
    engines = [
        value
        for value in documents
    ...
    if (
        identity.get("statefulset_name") != "epistemic-graph-raft"
        or identity.get("peer_service_name") != "epistemic-graph-raft"
        or identity.get("client_service_name") != "epistemic-graph-coordinator"
    ):
        raise ProductionAssetError(
            "engine identity contract names are not the canonical production names"
        )
    services = {
    ...
    peer_ports = {
        entry.get("name"): int(entry.get("port"))
        for entry in (peer_service.get("spec") or {}).get("ports") or ()
    }
    client_ports = {
        entry.get("name"): int(entry.get("port"))
        for entry in (client_service.get("spec") or {}).get("ports") or ()
    }
    if (
        peer_ports.get("rpc") != identity.get("client_port")
        or peer_ports.get("raft") != identity.get("peer_port")
        or peer_ports.get("metrics") != identity.get("metrics_port")
        or client_ports.get("rpc") != identity.get("client_port")
    ):
        raise ProductionAssetError(
            "engine Service ports disagree with the canonical identity contract"
        )
    engine = engines[0]
...
def _validate_worker_autoscaling(
    documents: list[dict[str, Any]], *, topology: dict[str, Any] | None = None
) -> None:
    expected_commands = {
    ...
    if topology_data:
        expected_metrics = {
            "graphos-front": str(
                topology_data.get("WORKLOAD_GATEWAY_METRIC_NAME") or ""
            ),
            "graphos-dispatch-worker": str(
                topology_data.get("WORKLOAD_DISPATCH_METRIC_NAME") or ""
            ),
            "graphos-ingest-worker": str(
                topology_data.get("WORKLOAD_INGEST_METRIC_NAME") or ""
            ),
            "graphos-analytics-worker": str(
                topology_data.get("WORKLOAD_MINING_METRIC_NAME") or ""
            ),
        }
        metric_modes = {
            "graphos-front": str(
                topology_data.get("WORKLOAD_GATEWAY_METRIC_MODE") or ""
            ),
            "graphos-dispatch-worker": str(
                topology_data.get("WORKLOAD_DISPATCH_METRIC_MODE") or ""
            ),
            "graphos-ingest-worker": str(
                topology_data.get("WORKLOAD_INGEST_METRIC_MODE") or ""
            ),
            "graphos-analytics-worker": str(
                topology_data.get("WORKLOAD_MINING_METRIC_MODE") or ""
            ),
        }
    else:
    ...
    for name, command in expected_commands.items():
        ...
        if topology_data:
            if (
                ...
                or (strategy.get("rollingUpdate") or {}).get("maxUnavailable") != 0
            ):
                raise ProductionAssetError(
    ...
    for name, metric_name in expected_metrics.items():
        ...
        if metric_modes[name] == "fixed":
            if hpa is not None:
                raise ProductionAssetError(
                    f"HorizontalPodAutoscaler/{name} must be absent for fixed scaling"
                )
            continue
        hpa = hpa or {}
        ...
        if topology_data:
            ...
            if not any(
                ...
                and ((metric.get("external") or {}).get("metric") or {})
                .get("selector", {})
                .get("matchLabels", {})
                .get("graphos_workload")
                == expected_workload
                for metric in metrics
            ):
                raise ProductionAssetError(
                    f"HorizontalPodAutoscaler/{name} lacks its graphos_workload selector"
                )
...
def check(directory: Path, *, rendered: bool, repository_root: Path) -> dict[str, Any]:
    ...
    topology_contract = next(
        (
            ...
            for value in documents
            if _identity(value)
            == ("ConfigMap", "graphos-topology-contract", "graphos-control")
        ),
        None,
    )
    ...
    if topology_contract is not None:
        if (
            topology_contract.get("immutable") is not True
            or (topology_contract.get("metadata") or {})
            .get("annotations", {})
            .get("graphos.io/authority-retention")
            != "Retain"
        ):
            raise ProductionAssetError(
                "topology contract must be immutable and retained"
            )
        ...
        if (
            str(
                (topology_contract.get("data") or {}).get("WORKLOAD_MINING_METRIC_MODE")
            )
            == "fixed"
        ):
            required_objects.discard(
                ("HorizontalPodAutoscaler", "graphos-analytics-worker", "graphos-cell")
            )
