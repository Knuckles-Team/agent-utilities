...
regenerates the CPD set from the canonical ToolSpec universe + EG ledger (or its
vendored cache) and fails if the checked-in documentation or packaged catalog
differs, so a CPD can never quietly rot relative to its sources.
Also asserts two structural invariants no textual diff alone would catch as
cleanly:

1. **Coverage** — every core, intent, and supported optional ToolSpec has
   exactly one CPD, and every CPD id names a canonical tool (no
   orphan/phantom CPD). The packaged catalog is universal so an enabled
   runtime feature can never appear without routing authority.
...
from agent_utilities.mcp.tool_specs import SUPPORTED_FEATURES  # noqa: E402
...
def _check_drift() -> tuple[list[str], list]:
    ...
    cpds, generated_at = gcp.generate(
        None,
        refresh_cache=False,
        prefer_cache=True,
    ...
    if strip_generation_timestamp(
        ...
        errors.append(
            f"{gcp.MD_PATH} is stale relative to the canonical ToolSpec universe + EG "
    ...
    if strip_generation_timestamp(
        ...
        errors.append(
            f"{gcp.JSON_PATH} is stale relative to the canonical ToolSpec universe + EG "
            "ledger — run `python scripts/gen_capability_power.py --write`."
        )
    if strip_generation_timestamp(
        ...
        errors.append(
            f"{gcp.PACKAGE_JSON_PATH} is stale relative to the canonical ToolSpec "
            "universe + EG ledger — run "
            "`python scripts/gen_capability_power.py --write`."
        )
...
def _check_coverage(cpds) -> list[str]:
    ...
    tool_names = set(canonical_tool_names(features=SUPPORTED_FEATURES))
