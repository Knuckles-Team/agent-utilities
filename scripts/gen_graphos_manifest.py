...
def build_manifest() -> list[dict]:
    ...
    if not ENGINE_DOMAINS:
        ...
        print(
            "WARNING: ENGINE_DOMAINS is empty — the 'epistemic_graph' client "
            "package did not import in this environment, so every "
            "engine_<domain>_<method> op will be missing from this manifest. "
            "Install epistemic-graph (with its numeric kernel) and regenerate.",
            file=sys.stderr,
        )
...
def _load_previous_manifest(path: Path) -> list[dict] | None:
    """Best-effort load of the CURRENTLY CHECKED-IN manifest's op list.

    Returns ``None`` only when the file doesn't exist yet (first-ever
    generation — nothing to regress against). Any other failure to read an
    existing file is a hard error: treating an unreadable baseline as "nothing
    to compare against" would reopen exactly the silent-shrink hole this
    guards against.
    """
    if not path.exists():
        return None
    ...
    spec = importlib.util.spec_from_file_location("_previous_graphos_manifest", path)
    if spec is None or spec.loader is None:  # pragma: no cover - defensive
        raise RuntimeError(f"could not load previous manifest for comparison: {path}")
...
def main() -> None:
    ...
    previous = _load_previous_manifest(out)

    ops = build_manifest()

    if previous is not None and len(ops) < len(previous):
        # Never silently emit a shrunken manifest (CONCEPT:AU-KG.compute.engine-surface-manifest) — a
        # regen that discovers FEWER ops than are already checked in almost
        # always means partial/optional-dep discovery failed quietly in this
        # environment, not that the tool surface genuinely shrank. Refuse to
        # write, name exactly what went missing, and exit nonzero so CI/an
        # operator sees a loud failure instead of a silently smaller manifest.
        previous_names = {entry["name"] for entry in previous}
        new_names = {entry["name"] for entry in ops}
        missing = sorted(previous_names - new_names)
        engine_hint = ""
        try:
            import epistemic_graph  # noqa: F401
        except Exception as exc:
            engine_hint = (
                "\n  LIKELY CAUSE: the 'epistemic_graph' package failed to import "
                f"({type(exc).__name__}: {exc}) — engine_tools._discover_domains() "
                "silently returns {} when this happens, dropping every "
                "engine_<domain>_<method> op. Install the epistemic-graph wheel "
                "(with its numeric kernel) in this environment and regenerate."
            )
        sys.stderr.write(
            f"REFUSING to write a shrunken manifest: {len(ops)} ops < "
            f"{len(previous)} currently checked in "
            f"({len(missing)} missing tool op(s)).{engine_hint}\n"
            f"Missing ops (showing up to 40 of {len(missing)}):\n  "
            + "\n  ".join(missing[:40])
            + "\n"
        )
        sys.exit(1)
