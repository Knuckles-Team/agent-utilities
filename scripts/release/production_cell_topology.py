...
def _safe_ref(value: Any, field: str) -> str:
    ...
    _require(
        not any(
            secret in lowered
            for secret in ("password", "token", "bearer", "secret", "private-key")
        ),
...
def _digest_image(value: Any, field: str) -> str:
    ...
    _require(
        ...
        f"{field} must be an immutable @sha256 image",
    )
    return value
...
def _release_image(manifest: dict[str, Any] | None, component: str) -> str | None:
    ...
    _require(
        separator and name and suffix == digest,
        f"release component {component} has mismatched artifact/digest",
    )
    return artifact
...
def _validate_engine_contract(value: Any) -> dict[str, Any]:
    ...
    _require(
        contract.get("apiVersion") == "agent-utilities.io/engine-identity/v1",
        "engine identity apiVersion is not supported",
    )
    _require(
        contract.get("$schema")
        == "https://agent-utilities.invalid/schemas/engine-identity-contract-v1.json",
        "engine identity schema is not canonical",
    )
    _require(
        contract.get("kind") == "EngineIdentityContract",
        "engine identity kind is not supported",
    )
    _require(
        contract.get("authority") == _ENGINE_CONTRACT_REF,
        "engine identity authority is not canonical",
    )
    _require(
        contract.get("contract_version") == "engine-identity.v1",
        "engine identity contract version is not supported",
    )
    digest = _string(contract.get("digest"), "engine_identity_contract.digest")
    _require(
        _ENGINE_CONTRACT_DIGEST_RE.fullmatch(digest) is not None,
        "engine identity digest must be sha256",
    )
    _require(
        digest == _canonical_engine_contract_digest(contract),
        "engine identity digest does not match canonical content",
    )
    namespace = _dns(contract.get("namespace"), "engine_identity_contract.namespace")
    statefulset = _dns(
        contract.get("statefulset_name"), "engine_identity_contract.statefulset_name"
    )
    peer = _dns(
        contract.get("peer_service_name"), "engine_identity_contract.peer_service_name"
    )
    client = _dns(
        contract.get("client_service_name"),
        "engine_identity_contract.client_service_name",
    )
    peer_port = _positive(
        contract.get("peer_port"), "engine_identity_contract.peer_port"
    )
    client_port = _positive(
        contract.get("client_port"), "engine_identity_contract.client_port"
    )
    metrics_port = _positive(
        contract.get("metrics_port"), "engine_identity_contract.metrics_port"
    )
    _require(
        len({peer_port, client_port, metrics_port}) == 3,
        "engine identity ports must be distinct",
    )
    _require(
        all(port <= 65535 for port in (peer_port, client_port, metrics_port)),
        "engine identity ports must be <= 65535",
    )
    tls_name = _string(
        contract.get("tls_server_name"), "engine_identity_contract.tls_server_name"
    )
    expected_tls = f"{peer}.{namespace}.svc.cluster.local"
    _require(
        tls_name == expected_tls,
        "engine identity TLS server name must identify the canonical peer Service",
    )
    sans = contract.get("tls_sans")
    _require(
        isinstance(sans, list) and sans and all(isinstance(item, str) for item in sans),
        "engine identity tls_sans must be a non-empty string list",
    )
    _require(
        tls_name in sans and len(sans) == len(set(sans)),
        "engine identity tls_sans must contain the canonical name without duplicates",
    )
    _require(
        contract.get("discovery_identity") == tls_name,
        "engine identity discovery identity must equal TLS server name",
    )
    _require(
        contract.get("transport") == "length-prefixed-msgpack-tls",
        "engine identity transport is not supported",
    )
    aliases = contract.get("compatibility_aliases")
    _require(
        isinstance(aliases, list) and aliases,
        "engine identity compatibility aliases must be explicit",
    )
    for index, alias in enumerate(aliases):
        alias = _closed(
            alias,
            f"engine_identity_contract.compatibility_aliases[{index}]",
            {
                "peer_service_name",
                "client_service_name",
                "namespace",
        ...
        _require(
            alias.get("status") == "migration-only"
            and alias.get("live_authority") is False,
            "engine identity aliases cannot be live authority",
        )
        _require(
            (
                alias.get("peer_service_name"),
                alias.get("client_service_name"),
                alias.get("namespace"),
            )
            != (peer, client, namespace),
            "engine identity alias must differ from canonical identity",
        )
    return {
...
def _validate_engine_binding(value: Any, contract: dict[str, Any]) -> None:
    ...
    _require(
        binding.get("ref") == contract["ref"],
        "engine identity binding ref does not match the canonical contract",
    )
    _require(
        binding.get("digest") == contract["digest"],
        "engine identity binding digest does not match the canonical contract",
    )
...
def _validate_metric(raw: Any, workload: str) -> dict[str, Any]:
    metric = _closed(
        raw,
        f"workloads.{workload}.metric",
        {"mode", "name", "target", "adapter_ref", "reason"},
    )
    mode = _string(metric.get("mode"), f"workloads.{workload}.metric.mode")
    if mode == "fixed":
        _require(
            workload == "mining",
            "only mining may use fixed scaling until its signal is bounded",
        )
        return {
            "mode": mode,
            "reason": _string(
                metric.get("reason"), f"workloads.{workload}.metric.reason"
            ),
        }
    ...
    _require(
        _METRIC_RE.fullmatch(name) is not None,
        f"workloads.{workload}.metric.name is unsafe",
    )
    _require(
        name == _EXPECTED_METRICS[workload],
        f"workloads.{workload} must use {_EXPECTED_METRICS[workload]!r}",
    )
    target = _string(metric.get("target"), f"workloads.{workload}.metric.target")
    _require(
        _TARGET_RE.fullmatch(target) is not None,
        f"workloads.{workload}.metric.target is unbounded",
    )
    return {
        "mode": mode,
        "name": name,
        "target": target,
        "adapter_ref": _safe_ref(
            metric.get("adapter_ref"), f"workloads.{workload}.metric.adapter_ref"
        ),
    }
...
def validate(
    ...
    *,
    ...
    pool = _closed(
        document.get("resource_pool"),
        "resource_pool",
        {"id", "measured_at", "evidence_ref", "nodes"},
    )
    pool_id = _string(pool.get("id"), "resource_pool.id")
    measured_at = _string(pool.get("measured_at"), "resource_pool.measured_at")
    _require(
        _RFC3339_RE.fullmatch(measured_at) is not None,
        "resource_pool.measured_at must be RFC3339",
    )
    pool_evidence = _safe_ref(pool.get("evidence_ref"), "resource_pool.evidence_ref")
    raw_nodes = pool.get("nodes")
    _require(
        isinstance(raw_nodes, list) and raw_nodes,
        "resource_pool.nodes must be non-empty",
    )
    ...
    for index, raw in enumerate(raw_nodes):
        raw = _closed(
            raw,
            f"resource_pool.nodes[{index}]",
            {"ordinal", "id", "hostname", "zone", "allocatable"},
        )
        ordinal = raw.get("ordinal")
        _require(
            isinstance(ordinal, int) and not isinstance(ordinal, bool) and ordinal >= 0,
            f"resource_pool.nodes[{index}].ordinal must be non-negative",
        )
        node_id = _string(raw.get("id"), f"resource_pool.nodes[{index}].id")
        hostname = _string(
            raw.get("hostname"), f"resource_pool.nodes[{index}].hostname"
        )
        zone = _string(raw.get("zone"), f"resource_pool.nodes[{index}].zone")
        _require(
            _HOST_RE.fullmatch(hostname) is not None,
            f"resource_pool.nodes[{index}].hostname is unsafe",
        )
        _require(
            node_id not in seen_ids
            and hostname not in seen_hosts
            and ordinal not in ordinals,
            f"duplicate node identity at resource_pool.nodes[{index}]",
        )
        alloc = _closed(
            raw.get("allocatable"),
            f"resource_pool.nodes[{index}].allocatable",
        ...
        node = {
            ...
            "zone": zone,
            "cpu": _positive(
                alloc.get("cpu_millicores"),
                f"resource_pool.nodes[{index}].allocatable.cpu_millicores",
            ),
            "memory": _positive(
                alloc.get("memory_bytes"),
                f"resource_pool.nodes[{index}].allocatable.memory_bytes",
            ),
        }
        nodes.append(node)
    ...
    _require(
        [node["ordinal"] for node in nodes] == list(range(len(nodes))),
        "resource_pool.node ordinals must be contiguous",
    )
    ...
    engine_client = _dns(
        engine.get("client_service_name"), "engine.client_service_name"
    )
    ...
    for field in (
        ...
        _require(
            actual_engine[field] == contract[field],
            f"engine.{field} disagrees with the canonical engine identity contract",
        )
    _require(
        engine_port <= 65535 and raft_port <= 65535 and metrics_port <= 65535,
        "engine ports must be <= 65535",
    )
    tls_name = _string(engine.get("tls_server_name"), "engine.tls_server_name")
    _require(
        tls_name == contract["tls_server_name"],
        "engine.tls_server_name must match the canonical engine identity contract",
    )
    _require(
        engine.get("tls_verified") is True,
        "engine.tls_verified must be true after preflight",
    )
    engine_discovery = _safe_ref(engine.get("discovery_ref"), "engine.discovery_ref")
    _require(
        engine.get("discovery_verified") is True,
        "engine.discovery_verified must be true after preflight",
    )
    engine_replicas = _positive(engine.get("replicas"), "engine.replicas")
    _require(
        engine_replicas == 3,
        "the production cell requires exactly three engine members",
    )
    _require(
        engine_replicas <= len(zones), "engine replicas exceed measured failure domains"
    )
    ...
    engine_rollback = _digest_image(
        engine.get("rollback_image"), "engine.rollback_image"
    )
    expected_engine = _release_image(release_manifest, "epistemic-graph")
    if expected_engine is not None:
        _require(
            engine_current == expected_engine,
            "engine.current_image does not match the exact release manifest",
        )
    engine_resources = _closed(
        engine.get("resources"),
        "engine.resources",
        {
    ...
    engine_cpu_request = _positive(
        engine_resources.get("cpu_request_millicores"),
        "engine.resources.cpu_request_millicores",
    )
    engine_cpu_limit = _positive(
        engine_resources.get("cpu_limit_millicores"),
        "engine.resources.cpu_limit_millicores",
    )
    engine_memory_request = _positive(
        engine_resources.get("memory_request_bytes"),
        "engine.resources.memory_request_bytes",
    )
    engine_memory_limit = _positive(
        engine_resources.get("memory_limit_bytes"),
        "engine.resources.memory_limit_bytes",
    )
    _require(
        engine_cpu_request <= engine_cpu_limit
        and engine_memory_request <= engine_memory_limit,
        "engine resource request must not exceed limit",
    )
    _require(
        all(
            node["cpu"] >= engine_cpu_limit and node["memory"] >= engine_memory_limit
            for node in nodes
        ),
        "measured node capacity cannot host engine limits",
    )
    engine_pdb = engine.get("pdb_max_unavailable")
    _require(
        isinstance(engine_pdb, int) and 0 <= engine_pdb < engine_replicas,
        "engine.pdb_max_unavailable must preserve quorum",
    )
    engine_termination = _positive(
        engine.get("termination_grace_seconds"), "engine.termination_grace_seconds"
    )
    engine_pre_stop = engine.get("pre_stop_seconds")
    _require(
        isinstance(engine_pre_stop, int) and 0 <= engine_pre_stop < engine_termination,
        "engine.pre_stop_seconds must be below termination grace",
    )

    oidc = _closed(
        document.get("oidc"),
        "oidc",
        {"issuer", "jwks_url", "audience", "client_id", "verified"},
    )
    ...
    _require(
        issuer_url.scheme == "https" and issuer_url.netloc, "oidc.issuer must be HTTPS"
    )
    _require(
        jwks.scheme == "https" and jwks.netloc == issuer_url.netloc,
        "oidc.jwks_url must share the HTTPS authority",
    )
    _require(
        not issuer_url.username
        and not issuer_url.password
        and not jwks.username
        and not jwks.password,
        "oidc URLs must not contain userinfo",
    )
    ...
    authorities = _closed(
        ...
        "authorities",
        {
            "control_configmap_name",
            "cell_configmap_name",
            "configmap_retention",
            "control_configmap_ref",
            "cell_configmap_ref",
            "secret_name",
            "secret_retention",
            "secret_ref",
    ...
    control_config = _dns(
        authorities.get("control_configmap_name"), "authorities.control_configmap_name"
    )
    cell_config = _dns(
        authorities.get("cell_configmap_name"), "authorities.cell_configmap_name"
    )
    _require(
        authorities.get("configmap_retention") == "Retain",
        "ConfigMap authority retention must be Retain",
    )
    control_config_ref = _safe_ref(
        authorities.get("control_configmap_ref"), "authorities.control_configmap_ref"
    )
    cell_config_ref = _safe_ref(
        authorities.get("cell_configmap_ref"), "authorities.cell_configmap_ref"
    )
    secret_name = _dns(authorities.get("secret_name"), "authorities.secret_name")
    _require(
        authorities.get("secret_retention") == "Retain",
        "Secret authority retention must be Retain",
    )
    secret_ref = _safe_ref(authorities.get("secret_ref"), "authorities.secret_ref")
    session = _closed(
        authorities.get("session_store"),
        "authorities.session_store",
        {"kind", "service_name", "namespace", "authority_ref", "retention"},
    )
    _require(
        session.get("kind") == "external",
        "authorities.session_store.kind must be external",
    )
    session_service = _dns(
        session.get("service_name"), "authorities.session_store.service_name"
    )
    session_namespace = _dns(
        session.get("namespace"), "authorities.session_store.namespace"
    )
    session_ref = _safe_ref(
        session.get("authority_ref"), "authorities.session_store.authority_ref"
    )
    _require(
        session.get("retention") == "Retain",
        "session store authority retention must be Retain",
    )
    action_audit = _closed(
        authorities.get("action_audit"),
        "authorities.action_audit",
        {"authority_ref", "retention", "graph", "kind"},
    )
    action_audit_ref = _safe_ref(
        action_audit.get("authority_ref"), "authorities.action_audit.authority_ref"
    )
    _require(
        action_audit.get("retention") == "Retain",
        "action audit authority retention must be Retain",
    )
    action_audit_graph = _string(
        action_audit.get("graph"), "authorities.action_audit.graph"
    )
    action_audit_kind = _string(
        action_audit.get("kind"), "authorities.action_audit.kind"
    )
    ...
    current_manifest_ref = _safe_ref(
        release.get("current_manifest_ref"), "release.current_manifest_ref"
    )
    rollback_manifest_ref = _safe_ref(
        release.get("rollback_manifest_ref"), "release.rollback_manifest_ref"
    )
    rollout_evidence = _safe_ref(
        release.get("rollout_evidence_ref"), "release.rollout_evidence_ref"
    )
    rollback_evidence = _safe_ref(
        release.get("rollback_evidence_ref"), "release.rollback_evidence_ref"
    )

    workloads = _closed(
        document.get("workloads"),
        "workloads",
        {"gateway", "dispatch", "ingest", "mining"},
    )
    expected_components = {
        "gateway": "agent-utilities",
        "dispatch": "agent-utilities",
        "ingest": "agent-utilities",
        "mining": "agent-utilities",
    }
    ...
    for workload_key in ("gateway", "dispatch", "ingest", "mining"):
        ...
        names.add(name)
        entrypoint = _string(
            raw.get("entrypoint"), f"workloads.{workload_key}.entrypoint"
        )
        _require(
            _TOKEN_RE.fullmatch(entrypoint) is not None,
            f"workloads.{workload_key}.entrypoint is unsafe",
        )
        component = _string(
            raw.get("component") or expected_components[workload_key],
            f"workloads.{workload_key}.component",
        )
        _require(
            component == expected_components[workload_key],
            f"workloads.{workload_key}.component must be {expected_components[workload_key]!r}",
        )
        current_image = _digest_image(
            raw.get("current_image"), f"workloads.{workload_key}.current_image"
        )
        rollback_image = _digest_image(
            raw.get("rollback_image"), f"workloads.{workload_key}.rollback_image"
        )
        expected_image = _release_image(release_manifest, component)
        if expected_image is not None:
            _require(
                current_image == expected_image,
                f"workloads.{workload_key}.current_image does not match the exact release manifest",
            )
        replicas = _closed(
            raw.get("replicas"),
            f"workloads.{workload_key}.replicas",
            {
                "min",
                "desired",
                "max",
                "scale_up_step",
                "scale_down_step",
                "up_stabilization_seconds",
                "down_stabilization_seconds",
            },
        )
        minimum = _positive(
            replicas.get("min"), f"workloads.{workload_key}.replicas.min"
        )
        desired = _positive(
            replicas.get("desired"), f"workloads.{workload_key}.replicas.desired"
        )
        maximum = _positive(
            replicas.get("max"), f"workloads.{workload_key}.replicas.max"
        )
        _require(
            minimum <= desired <= maximum and maximum <= len(zones),
            f"workloads.{workload_key}.replicas must fit min <= desired <= max <= measured zones",
        )
        scale_up = _positive(
            replicas.get("scale_up_step"),
            f"workloads.{workload_key}.replicas.scale_up_step",
        )
        scale_down = _positive(
            replicas.get("scale_down_step"),
            f"workloads.{workload_key}.replicas.scale_down_step",
        )
        up_window = replicas.get("up_stabilization_seconds")
        down_window = replicas.get("down_stabilization_seconds")
        _require(
            isinstance(up_window, int)
            and up_window >= 0
            and isinstance(down_window, int)
            and down_window >= 0,
            f"workloads.{workload_key}.stabilization windows must be non-negative",
        )
        resources = _closed(
            raw.get("resources"),
            f"workloads.{workload_key}.resources",
            {
                "cpu_request_millicores",
                "cpu_limit_millicores",
                "memory_request_bytes",
        ...
        cpu_request = _positive(
            resources.get("cpu_request_millicores"),
            f"workloads.{workload_key}.resources.cpu_request_millicores",
        )
        cpu_limit = _positive(
            resources.get("cpu_limit_millicores"),
            f"workloads.{workload_key}.resources.cpu_limit_millicores",
        )
        memory_request = _positive(
            resources.get("memory_request_bytes"),
            f"workloads.{workload_key}.resources.memory_request_bytes",
        )
        memory_limit = _positive(
            resources.get("memory_limit_bytes"),
            f"workloads.{workload_key}.resources.memory_limit_bytes",
        )
        _require(
            cpu_request <= cpu_limit and memory_request <= memory_limit,
            f"workloads.{workload_key}.resource request must not exceed limit",
        )
        _require(
            all(
                node["cpu"] >= cpu_limit and node["memory"] >= memory_limit
                for node in nodes
            ),
            f"measured node capacity cannot host {workload_key} limits",
        )
        ...
        _require(
            isinstance(pdb, int) and 0 <= pdb < minimum,
            f"workloads.{workload_key}.pdb_max_unavailable must preserve one replica",
        )
        termination = _positive(
            raw.get("termination_grace_seconds"),
            f"workloads.{workload_key}.termination_grace_seconds",
        )
        pre_stop = raw.get("pre_stop_seconds")
        _require(
            isinstance(pre_stop, int) and 0 <= pre_stop < termination,
            f"workloads.{workload_key}.pre_stop_seconds must be below termination grace",
        )
        metric = _validate_metric(raw.get("metric"), workload_key)
        expected_name, expected_namespace, expected_entrypoint = _EXPECTED_WORKLOADS[
            workload_key
        ]
        _require(
            name == expected_name,
            f"workloads.{workload_key}.name must be the canonical production-cell identity {expected_name!r}",
        )
        _require(
            namespace == expected_namespace,
            f"workloads.{workload_key}.namespace must be the canonical production-cell namespace {expected_namespace!r}",
        )
        _require(
            entrypoint == expected_entrypoint,
            f"workloads.{workload_key}.entrypoint must be {expected_entrypoint!r}",
        )
        normalized[workload_key] = {
            ...
            "pdb_min_available": minimum - pdb,
            "termination": termination,
            "pre_stop": pre_stop,
            "metric": metric,
        }
    _require(
        total_cpu_request <= sum(node["cpu"] for node in nodes),
        "maximum workload CPU requests exceed measured pool",
    )
    _require(
        total_cpu_limit <= sum(node["cpu"] for node in nodes),
        "maximum workload CPU limits exceed measured pool",
    )
    _require(
        total_memory_request <= sum(node["memory"] for node in nodes),
        "maximum workload memory requests exceed measured pool",
    )
    _require(
        total_memory_limit <= sum(node["memory"] for node in nodes),
        "maximum workload memory limits exceed measured pool",
    )
    gateway_images = (
        normalized["gateway"]["current_image"],
        normalized["gateway"]["rollback_image"],
    )
    ...
    topology = {
        ...
        "nodes": nodes,
        "zones": sorted(zones),
        "engine_statefulset": engine_statefulset,
        "engine_client": engine_client,
        "engine_peer": engine_peer,
        "engine_namespace": engine_namespace,
        "engine_port": engine_port,
        "raft_port": raft_port,
        "engine_metrics_port": metrics_port,
        "engine_endpoint": f"tls://{engine_client}.{engine_namespace}.svc.cluster.local:{engine_port}",
        "engine_tls_name": tls_name,
        "engine_discovery": engine_discovery,
        "engine_identity_ref": contract["ref"],
        "engine_identity_digest": contract["digest"],
        "engine_replicas": engine_replicas,
        "engine_raft_groups": raft_groups,
        "engine_current_image": engine_current,
        "engine_rollback_image": engine_rollback,
        "engine_cpu_request": str(engine_cpu_request),
        "engine_cpu_limit": str(engine_cpu_limit),
        "engine_memory_request": str(engine_memory_request),
        "engine_memory_limit": str(engine_memory_limit),
        "engine_pdb_max_unavailable": engine_pdb,
        "engine_termination": engine_termination,
        "engine_pre_stop": engine_pre_stop,
        "oidc_issuer": issuer,
        "oidc_jwks": jwks_url,
        ...
        "control_config_ref": control_config_ref,
        "cell_config_ref": cell_config_ref,
        ...
        "action_audit_graph": action_audit_graph,
        "action_audit_kind": action_audit_kind,
    ...
    topology["input_digest"] = hashlib.sha256(
...
def canonical_contract(topology: dict[str, Any], *, rollback: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "data": {
            ...
            **{
                f"WORKLOAD_{key.upper()}_ADAPTER_REF": item["metric"].get(
                    "adapter_ref", ""
                )
                for key, item in topology["workloads"].items()
            },
...
def _set_container_contract(
    container: dict[str, Any], item: dict[str, Any], *, rollback: bool
) -> None:
    resources = container.setdefault("resources", {})
    resources["requests"] = {
        "cpu": _resource_value(item["cpu_request"]),
        "memory": _resource_value(item["memory_request"]),
    }
    resources["limits"] = {
        "cpu": _resource_value(item["cpu_limit"]),
        "memory": _resource_value(item["memory_limit"]),
    }
    container["lifecycle"] = {
        "preStop": {"exec": {"command": ["/bin/sh", "-c", f"sleep {item['pre_stop']}"]}}
    }
    if rollback:
...
def _set_named_port(document: dict[str, Any], name: str, port: int) -> None:
    ...
    raise TopologyInputError(
        f"template is missing named port {name!r} for {_identity(document)}"
    )


def apply_to_documents(
    documents: list[dict[str, Any]], topology: dict[str, Any], *, rollback: bool
) -> list[dict[str, Any]]:
    ...
    _require(
        by_identity.get(("ConfigMap", topology["control_config"], "graphos-control"))
        is not None,
        "template is missing the declared control ConfigMap authority",
    )
    _require(
        by_identity.get(("ConfigMap", topology["cell_config"], "graphos-cell"))
        is not None,
    ...
    for workload_key, identity in _WORKLOAD_DOCS.items():
        ...
        spec["strategy"] = {
            "type": "RollingUpdate",
            "rollingUpdate": {"maxSurge": item["scale_up"], "maxUnavailable": 0},
        }
        ...
        _require(
            len(containers) == 1, f"{identity} must have one application container"
        )
        ...
        pod_annotations = template.setdefault("metadata", {}).setdefault(
            "annotations", {}
        )
    ...
    engine_identity = (
        "StatefulSet",
        topology["engine_statefulset"],
        topology["engine_namespace"],
    )
    ...
    engine_container["resources"] = {
        "requests": {
            "cpu": topology["engine_cpu_request"],
            "memory": topology["engine_memory_request"],
        },
        "limits": {
            "cpu": topology["engine_cpu_limit"],
            "memory": topology["engine_memory_limit"],
        },
    }
    engine_container["lifecycle"] = {
        "preStop": {
            "exec": {
                "command": ["/bin/sh", "-c", f"sleep {topology['engine_pre_stop']}"]
            }
        }
    }
    if rollback:
    ...
    engine_env = {
        ...
        for entry in engine_container.get("env") or ()
    ...
    for workload_key, identity in _WORKLOAD_DOCS.items():
        ...
        pdb = next(
            (
                document
                for document in documents
                if _identity(document) == ("PodDisruptionBudget", pdb_name, identity[2])
            ),
            None,
        )
    ...
    engine_pdb.setdefault("spec", {})["maxUnavailable"] = topology[
        "engine_pdb_max_unavailable"
    ]

    hpa_names = {
        "gateway": "graphos-front",
        "dispatch": "graphos-dispatch-worker",
        "ingest": "graphos-ingest-worker",
        "mining": "graphos-analytics-worker",
    }
    for workload_key, name in hpa_names.items():
        ...
        hpa = next(
            (
                ...
                for document in documents
                if _identity(document)
                == ("HorizontalPodAutoscaler", name, _WORKLOAD_DOCS[workload_key][2])
            ),
            None,
        )
        ...
        hpa_spec["behavior"] = {
            "scaleUp": {
                "stabilizationWindowSeconds": item["up_window"],
                "selectPolicy": "Max",
                "policies": [
                    {"type": "Pods", "value": item["scale_up"], "periodSeconds": 60}
                ],
            },
            "scaleDown": {
                "stabilizationWindowSeconds": item["down_window"],
                "selectPolicy": "Max",
                "policies": [
                    {"type": "Pods", "value": item["scale_down"], "periodSeconds": 60}
                ],
            },
        }
        ...
        hpa_spec["metrics"] = [
            {
                ...
                "external": {
                    "metric": {
                        ...
                        "selector": {"matchLabels": {"graphos_workload": workload_key}},
                    },
                    "target": {
                        "type": "AverageValue",
    ...
    for document in documents:
        ...
        if identity in {
            ...
            document.setdefault("metadata", {}).setdefault("annotations", {}).update(
                {
                    "graphos.io/authority-ref": topology["control_config_ref"]
                    if identity[1] == topology["control_config"]
                    else topology["cell_config_ref"],
                    "graphos.io/authority-retention": "Retain",
                    "graphos.io/topology-input-digest": topology["input_digest"],
                }
            )
            ...
            if identity[1] == topology["cell_config"]:
                data["EPISTEMIC_GRAPH_RAFT_GROUPS"] = str(
                    topology["engine_raft_groups"]
                )
        if identity == (
            "ConfigMap",
            "graphos-prometheus-adapter-rules",
            "graphos-control",
        ):
            document.setdefault("metadata", {}).setdefault("annotations", {}).update(
                {
            ...
            document.setdefault("data", {})["external-rules.yaml"] = _adapter_rules(
                topology
            )
    return documents
...
def _adapter_rules(topology: dict[str, Any]) -> str:
    ...
    for key in ("gateway", "dispatch", "ingest"):
        ...
        rules.append(
            "      - seriesQuery: '{name}'\n"
            "        name: {{matches: '^{name}$'}}\n"
            "        metricsQuery: 'sum({name}{{<<.LabelMatchers>>}})'".format(
                name=metric["name"]
            )
        )
