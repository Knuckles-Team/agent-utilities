...
def validate_mermaid_block(filepath, start_line, block_lines):
    ...
    for idx, (ln, l) in enumerate(block_lines):  # noqa: E741
        l_clean = l.strip()
        if not l_clean:
            continue
    ...
    for ln, l in active_lines:  # noqa: E741
        l_clean = l.strip()
...
def main():
    ...
    if total_violations > 0:
        print("\n" + "=" * 80)
        print(
            f"MERMAID DIAGRAM SYNTAX VERIFICATION FAILED: Found {total_violations} syntax errors!"
        )
        print("=" * 80)
