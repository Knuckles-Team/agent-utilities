...
def _validate_written_bundle(
    ...
    *,
    ...
    # name, which can legitimately equal a connector identity (for example the
    # ``genius-agent`` provider generated by the ``genius`` account). Static
    # credential, address, and path patterns remain active with an explicit empty
    # runtime deny-list.
    privacy = PersistencePrivacyGuard(deny_terms=())
    semantic_paths = {
        shapes_path,
        module / "ontology" / "mappings" / "source.yaml",
        fixture_path,
