...
class TestCodebaseArtifactRouting:
    ...
    async def test_routes_each_type(self, mixed_repo: Path, monkeypatch):
        ...
        # Spy on the central ingest seam: record each routed sub-manifest and
        # return a success result carrying a stable source_id (for repo linking).
        ...
        by_type: dict[str, list[Path]] = {"skill": [], "prompt": [], "document": []}
        for m in seen:
        ...
        # Each markdown routed to DOCUMENT adaptor (incl. the one in a code dir).
        assert (mixed_repo / "README.md") in by_type["document"]
        assert (mixed_repo / "src" / "notes.md") in by_type["document"]
        # The skill's nested ref.md was claimed → never routed as a Document.
        assert (mixed_repo / "skills" / "myskill" / "reference" / "ref.md") not in by_type[
            "document"
        ]
