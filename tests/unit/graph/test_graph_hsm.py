...
def test_static_route_query():
    adaptive_agent_router = {
        ...
        "git_expert": "Git operations",
    }

    assert (
        static_route_query("Please run researcher", adaptive_agent_router)
        == "researcher"
    )
    assert (
        static_route_query("Use git expert here", adaptive_agent_router) == "git_expert"
    )
