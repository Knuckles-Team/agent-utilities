...
async def test_on_exit_specialist():
    ...
    with patch("agent_utilities.graph.hsm.MCPServerHealth") as mock_health_class:
        ...
        await executor.on_exit_specialist(
            deps, state, "test_agent", success=True, server_name="test_server"
        )
...
async def test_execute_specialized_step():
    ...
    with (
        ...
        patch(
            "agent_utilities.graph.executor._get_domain_tools",
        ...
        patch(
            "agent_utilities.graph.executor.on_exit_specialist", new_callable=AsyncMock
        ),
...
async def test_execute_agent_package_logic_local():
    ...
    with (
        ...
        patch(
            "agent_utilities.graph.executor._execute_dynamic_mcp_agent",
            new_callable=AsyncMock,
        ) as mock_exec,
    ):
