...
route_capability_request`, against a fake engine-native ANN surface. All four X-4 acceptance
...
def _make_engine(nodes: dict[str, dict[str, Any]]) -> Any:
    """A fake engine-native ANN and durable backend sharing one node store."""
...
def test_subsumption_is_on_by_default_without_an_explicit_hierarchy_argument():
    """Every X-4 entry point resolves the bundled hierarchy by default.

    Omitting ``capability_hierarchy`` must still surface the subsuming tool and
    never silently degrade to flat exact matching.
    """
    nodes = {
        "mtls_tool": {
            "type": "tool",
