...
stands in for the real ``RunTrace -[:USED_TOOL]-> ToolCall`` / attribution-reference
``-[:PRODUCED_OUTCOME]-> OutcomeEvaluation`` subgraph (mirroring the ``FakeEngine``
idiom used by the optimizer tests, so we can assert a failing trace becomes a labeled
...
from agent_utilities.harness.program_optimization import get_target
...
class FakeToolEngine:
    """Stands in for the live engine on the ``RunTrace -[:USED_TOOL]-> ToolCall`` path."""
...
class FakeTagEngine:
    ...
    def query_cypher(self, cypher, params=None):
        self.calls.append((cypher, params or {}))
        assert "attribution_ref" in cypher
        assert params.get("agent_ref")
        assert params.get("skill_ref")
        return self.rows
...
def test_gather_trace_examples_seeds_a_failing_episode_as_negative_example():
    """A failing :RunTrace -[:PRODUCED_OUTCOME]-> :OutcomeEvaluation for the target
...
def test_record_trace_derived_finding_persists_when_engine_supports_add_node():
    ...
    stats = {
        "component_type": "tool_description",
        "identifier_ref": "pref_optimization_target_" + "a" * 64,
        "trace_derived": 1,
        "trace_failures": 1,
        "trace_successes": 0,
        "self_supervised": 1,
    ...
    assert label == "ProgramOptimizationFinding"
    assert props["trace_derived_count"] == 1
    assert "identifier" not in props
    assert "my_tool" not in repr(engine.nodes)
...
def test_record_trace_derived_finding_noop_without_traces_or_engine():
    ...
    full_stats = {
        "component_type": "x",
        "identifier_ref": "pref_optimization_target_" + "b" * 64,
        "trace_derived": 1,
        "trace_failures": 1,
        "trace_successes": 0,
        "self_supervised": 0,
