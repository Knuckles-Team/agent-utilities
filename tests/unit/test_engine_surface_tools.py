...
def test_kg_2_310_logs_query_dispatches(monkeypatch, tools):
    """CONCEPT:AU-KG.coordination.engine-message-broker — graph_logs routes 'query' into client.observability.search_logs."""
    calls: list = []
    obs = SimpleNamespace(search_logs=_recording_method(calls, "search_logs"))
    monkeypatch.setattr(
        engine_surface_tools, "_client", lambda graph: _fake_client(observability=obs)
    )
    out = json.loads(
