...
def test_graph_viz_capability_matrix_dispatches(monkeypatch, tools):
    ...
    monkeypatch.setattr(
        engine_surface_tools, "_client", lambda graph: _fake_client(viz=viz)
    )
...
def test_graph_viz_export_chart_dispatches_and_round_trips_bytes(monkeypatch, tools):
    ...
    dataset = {
        "InlineColumns": {
            "columns": {"x": {"F64": [1.0, 2.0]}, "y": {"F64": [3.0, 4.0]}}
        }
    }
    out = json.loads(
        tools["graph_viz"](
...
def test_graph_viz_export_chart_requires_both_spec_and_dataset(tools):
    out = json.loads(
        tools["graph_viz"](
            **_viz_args(action="export_chart", spec_json="", dataset_json="")
        )
    )
    assert "error" in out
...
def test_graph_viz_plot_from_query_renders_real_query_rows(monkeypatch, tools):
    ...
    assert render_calls[0][1] == {
        "InlineColumns": {
            "columns": {"a": {"F64": [1.0, 2.0, 3.0]}, "b": {"F64": [10.0, 20.0, 30.0]}}
        }
    }
    assert render_calls[0][0]["marks"][0]["kind"] == "scatter"


def test_graph_viz_plot_from_query_missing_data_is_unavailable_not_a_fabricated_chart(
    monkeypatch, tools
):
...
def test_graph_viz_plot_from_query_row_limit_is_clamped_to_a_hard_max(
    monkeypatch, tools
):
...
def test_graph_viz_plot_from_query_requires_fields(tools):
    out = json.loads(
        tools["graph_viz"](
            **_viz_args(
                action="plot_from_query", query="SELECT 1", x_field="", y_field=""
            )
        )
    )
    assert "error" in out
...
def test_graph_viz_describe_chart_summarizes_an_exact_render_without_any_engine_call(
    monkeypatch, tools
):
...
def test_graph_viz_describe_chart_flags_an_approximated_render(tools):
    ...
    out = json.loads(
        tools["graph_viz"](
            **_viz_args(
                action="describe_chart", view_result_json=json.dumps(view_result)
            )
        )
    )
...
def test_graph_viz_describe_chart_requires_view_result(tools):
    out = json.loads(
        tools["graph_viz"](**_viz_args(action="describe_chart", view_result_json=""))
    )
    assert "error" in out
