...
async def test_ctx_confirm_destructive():
    ...
    res = await context_helpers.ctx_confirm_destructive(mock_ctx, "delete something")
    assert res is True
    ...
    # Missing context does not grant unattended write authority.
    res = await context_helpers.ctx_confirm_destructive(None, "delete something")
    assert res is False

    # Elicitation failure is fail-closed.
...
def test_ctx_log():
    ...
    # Test info level
    context_helpers.ctx_log(mock_ctx, mock_logger, "info", "test message")
    ...
    mock_ctx.reset_mock()
    context_helpers.ctx_log(mock_ctx, mock_logger, "error", "error message")
    mock_logger.error.assert_called_once_with("error message")
    mock_ctx.error.assert_called_once_with("error message")

    # Test with ctx=None
    mock_logger.reset_mock()
    context_helpers.ctx_log(None, mock_logger, "info", "no ctx message")
    ...
    mock_ctx = MagicMock()
    del mock_ctx.debug
    context_helpers.ctx_log(mock_ctx, mock_logger, "debug", "debug message")
...
async def test_ctx_state_management():
    ...
    # Test set_state
    await context_helpers.ctx_set_state(mock_ctx, "testproj", "mykey", "myval")
    ...
    val = await context_helpers.ctx_get_state(mock_ctx, "testproj", "mykey")
    ...
    val = await context_helpers.ctx_get_state(
        mock_ctx, "testproj", "missing", default="fallback"
    )
    assert val == "fallback"

    # Test with ctx=None
    await context_helpers.ctx_set_state(None, "p", "k", "v")
    val = await context_helpers.ctx_get_state(None, "p", "k", default="def")
    assert val == "def"
