...
# D-DSTO-5 (reports/deferred/lane-dst-orch.md): these tests targeted the
# deleted KG-:AgentLease-node claim path with a fake engine shaped for its
# raw query_cypher contract; the same claim/lease/fencing/reclaim behavior
# against the CURRENT WorkItem-native contract is fully covered by
# tests/unit/test_engine_claim.py (backend delegation) and
# tests/unit/test_agent_dispatch_work_item_backend.py (claim/lease/dependency
# -release semantics) — removed here rather than re-authored against a
# different engine contract to avoid duplicating that coverage.
