...
def test_record_inbound_persists_pending_idempotently():
    eng = _FakeEngine()
    iid = record_inbound(eng, platform="telegram", channel_id="42", message_id="1", text="hi", session="s")
    ...
    iid2 = record_inbound(eng, platform="telegram", channel_id="42", message_id="1", text="hi", session="s")
    assert iid2 == iid


def test_mark_answered_sets_status():
    eng = _FakeEngine()
    iid = record_inbound(eng, platform="t", channel_id="c", message_id="1", text="hi", session="s")
    mark_answered(eng, iid)
    assert eng.nodes[iid]["status"] == "answered"


def test_pending_unanswered_honors_grace_window():
    eng = _FakeEngine(pending=[
        {"id": "a", "received_at": _old_ts(1000), "attempts": 0},   # old → eligible
        {"id": "b", "received_at": _old_ts(5), "attempts": 0},      # fresh → skip (grace)
    ])
...
async def test_retry_unanswered_answers_then_bumps_then_dead_letters():
    eng = _FakeEngine(pending=[{"id": "x", "received_at": _old_ts(1000), "attempts": 0}])
    ...
    eng2 = _FakeEngine(pending=[{"id": "y", "received_at": _old_ts(1000), "attempts": 2}])
    ...
    eng3 = _FakeEngine(pending=[{"id": "z", "received_at": _old_ts(1000), "attempts": 4}])
