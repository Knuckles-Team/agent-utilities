...
from agent_utilities.graph.models import Source, Concept, Evidence, Person, KnowledgeBaseTopic
...
async def test_concept_merging():
    ...
    with patch("agent_utilities.knowledge_graph.engine.cosine_similarity", return_value=0.99):
        merged = maintainer.merge_similar_concepts(similarity_threshold=0.9)
        assert merged == 1
        # Verify delete was called for c2
        mock_backend.execute.assert_any_call("MATCH (old:Concept {id: $old_id}) DETACH DELETE old", {"old_id": "c2"})
