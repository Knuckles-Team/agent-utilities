...
from agent_utilities.knowledge_graph.core.query_router import QueryRouter, QueryTier, QueryType
...
def test_query_router_cost_heuristics():
    ...
    tier = router._select_tier(QueryType.FILTERED_MATCH, expected_hops=1, requires_freshness=True)
    assert tier == QueryTier.L3_PERSISTENT

    # 2. Freshness for Topological hits L1
    tier = router._select_tier(QueryType.TOPOLOGICAL, expected_hops=2, requires_freshness=True)
    assert tier == QueryTier.L1_RUST

    # 3. Complex multi-hop hits L2 cache if available
    tier = router._select_tier(QueryType.FILTERED_MATCH, expected_hops=3, requires_freshness=False)
