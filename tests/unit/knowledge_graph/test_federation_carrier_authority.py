"""Carrier-authority regression test for the federation reader (BUG-005 / GOC-15).
...
(`plans/graph-os-completion-program/decisions/GOC-15-carrier-authority.md`) covers
this surface's fail-closed requirement.
`FederationMixin.execute_federated_query` / `_execute_federated_connection`
(`agent_utilities/knowledge_graph/orchestration/engine_federation.py`) now calls
`resolve_session`/`current_session` before reaching a backend's `execute_read`,
matching every other `graph_query` dialect: `cypher` explicitly requires
`kg:read` (`engine_query.py:185`) and `sql`/`sparql` are gated by the
session-routed wire client (`graph_compute.py`'s `_SessionRoutedAsyncClient._send()`).
...
audience context fails closed"). The regression remains green only while the
federation reader is wired to `resolve_session(required_scope=...)` before
touching any backend, per BUG-005's remediation design ("one
...
class _StubFederatedEngine:
    """Stands in for an injected registry's `get_engine(connection)` result.

    This is the exact shape `_execute_federated_connection` calls into
    (`engine_federation.py:364-381`): ``getattr(engine, "backend", None) or
...
def test_federated_query_denies_without_a_verified_session():
    ...
    `SessionRequiredError` before invoking the backend.
    """
    ...
    class _Registry:
        ...
        def get_engine(self, _connection: str) -> _StubFederatedEngine:
            self.calls += 1
            return stub_engine
    ...
    assert stub_engine.calls == 0, (
        "BUG-005/GOC-15 regression: the federation reader executed a backend read "
    ...
    assert registry.calls == 0, "registry access must follow session verification"
