...
Covers:
  * The full reason → downfeed → SHACL path. Reasoning is engine-native first
    (``client.rdf.owl_reason``, CONCEPT:KG-2.204) with a pure-Python RDFS+ closure
...
def test_run_closure_materializes_inferred_edges():
    ...
    # CONCEPT:KG-2.204 — engine-native closure needs no owlready2 promotion, so
...
def test_run_closure_runs_without_owlready2(monkeypatch):
    """CONCEPT:KG-2.204 — the closure reasons engine-native (or via the Python
