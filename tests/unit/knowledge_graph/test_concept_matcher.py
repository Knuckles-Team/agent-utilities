...
class _Engine:
    ...
    def link_nodes(self, src: str, dst: str, rel: Any, properties=None):
        self.graph._edges.append(
            (
                src,
                dst,
                {"_rel": str(getattr(rel, "value", rel) or rel), **(properties or {})},
            )
        )
    ...
    def delete_edge(self, src: str, dst: str, rel: str):
        self.graph._edges = [
            (s, t, p)
            for s, t, p in self.graph._edges
...
def test_explicit_id_match_is_covered():
    ...
    assert any(
        p.get("_rel") == "SATISFIED_BY" and p.get("match") == "id"
        for *_, p in eng.graph._edges
    )
...
def test_llm_judge_covered_writes_satisfied_by():
    nodes = {
        ...
        "article:p1": {
            "type": "article",
            "name": "Self-improving assimilation",
            "embedding": [0.99, 0.1],
        },
    }
    eng = _Engine(nodes)
    ...
    def judge(_p):
        return '{"verdict":"covered","confidence":0.95,"why":"same"}'
...
def test_llm_judge_related_writes_relates_to_and_stays_novel():
    ...
    def judge(_p):
        return '{"verdict":"related","confidence":0.8,"why":"adjacent"}'
