...
def test_writes_are_flushed_as_bulk_ops_nodes_before_edges():
    ...
    for i in range(5):
        bw.add_node(f"code:{i}", label="Code", language="java")
...
def test_source_system_stamped_on_every_node_when_configured():
    ...
    bw.add_node("code:x", label="Code", language="python")
    bw.add_node(
        "code:y", label="Test"
...
def test_auto_flush_at_batch_size():
    ...
    bw.add_node("a", label="Node")
    bw.add_node("b", label="Node")  # hits batch_size → auto-flush
...
def test_falls_back_to_per_item_without_bulk_path():
    ...
    bw.add_node("a", label="Node")
    bw.add_edge("a", "b", rel_type="LINKS")
    bw.flush()
