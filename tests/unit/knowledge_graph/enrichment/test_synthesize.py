...
def fake_capability_search(query, k):
    ...
    return [
        ...
        {
            "id": "skill:graph-ingestion-and-integration",
            "type": "Skill",
...
def test_synthesize_agent_grounds_in_candidates():
    def fake_llm(prompt):
        ...
        assert "graph_query" in prompt and "graph-ingestion-and-integration" in prompt
        return json.dumps(
            {
                "name": "Retriever Bot",
                "system_prompt": "You retrieve answers from the KG.",
                "tools": ["graph_query", "embed"],
                "skills": ["graph-ingestion-and-integration"],
    ...
    assert spec.skills == ["graph-ingestion-and-integration"]
