...
def test_maps_rust_parse_result_to_entities():
    parsed = {
        "nodes": [
            _fn("test_mock_heavy", True, assert_count=1, mock_count=3, fixture_count=2,
                calls="MagicMock,compute"),
            _fn("compute", False, file_path="app.py"),
            {"node_id": "symbol:C", "node_type": "SYMBOL",
             "properties": {"symbol_type": "Class", "name": "Widget",
                            "line": "1", "ast_hash": "h", "file_path": "app.py"}},
        ]
    }
...
def test_classify_mock_heavy_and_dormant_and_assertionfree():
    ...
    dormant = entities_from_parse_result(
        "test_t.py", "h", {"nodes": [_fn("test_b", True, is_skipped="true", marks="skip")]}
    ).tests[0]
    assert {i.code for i in classify_test(dormant, th)} == {"DormantTest"}

    healthy = entities_from_parse_result(
        "test_t.py", "h", {"nodes": [_fn("test_c", True, assert_count=3, mock_count=0, fixture_count=1)]}
    ).tests[0]
