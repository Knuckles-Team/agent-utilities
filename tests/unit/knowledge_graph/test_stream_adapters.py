...
def _cfg(**kw):
    base = dict(stream_id="s1", name="company-brain", source_type="kafka",
...
def test_kafka_adapter_consumes_injected_records():
    recs = [
        SimpleNamespace(value=json.dumps(
            {"event_id": "e1", "payload": {"x": 1}}).encode(), offset=0),
        SimpleNamespace(value=json.dumps(
            {"event_id": "e2", "event_type": "ticket", "payload": {"y": 2}}).encode(),
            offset=1),
    ]
...
def test_nats_adapter_consumes_and_acks():
    ...
    adapter = NatsStreamAdapter(_cfg(source_type="nats"), subscription=FakeNatsSub(msgs))
...
def test_make_stream_adapter_selects_by_source_type():
    assert isinstance(make_stream_adapter(_cfg(source_type="kafka")), KafkaStreamAdapter)
    assert isinstance(make_stream_adapter(_cfg(source_type="nats")), NatsStreamAdapter)


def test_decode_tolerates_non_json():
    adapter = KafkaStreamAdapter(_cfg(), consumer=FakeKafkaConsumer(
        [SimpleNamespace(value=b"not json", offset=0)]))
