...
class TestWatcherCPU(unittest.TestCase):
    ...
    def test_process_plan_file_mtime_caching(self):
        ...
        with (
            ...
            patch(
                "agent_utilities.sdd.watcher._get_latest_version_from_history",
                return_value=(0, set()),
            ),
            patch(
                "agent_utilities.sdd.watcher._parse_plan_metadata",
                return_value={"title": "Plan X", "approach": "Simple"},
            ),
            patch("agent_utilities.sdd.watcher.ingest_plan_version"),
            patch.object(
                Path, "read_text", return_value="title: Plan X\napproach: Simple"
            ) as mock_read,
        ):
    ...
    def test_process_tasks_file_mtime_caching(self):
        ...
        with (
            ...
            patch("agent_utilities.sdd.watcher.ingest_tasks_version"),
            patch.object(
                Path, "read_text", return_value="- [ ] Task 1\n- [ ] Task 2"
            ) as mock_read,
        ):
