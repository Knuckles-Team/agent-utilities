"""Live-path test: `graph_query`'s sole typed EvidenceBundle response wires the
...
`nl_query` and focused analysis tools use. On the single-connection local Cypher
path, the required response currency-upgrades the
...
typed EvidenceBundle. There is no raw response mode.
"""
...
def test_graph_query_wires_typed_evidence_bundle_from_engine_wire(
    monkeypatch,
):
    ...
    # Raw rows remain inspectable in the bundle trace.
    ...
    assert {c["id"] for c in out["claims"]} == {"agent:foo", "agent:bar"}
    ...
    assert out["freshness"]["valid_time"] == {"min": [50, None], "max": [100, None]}


def test_graph_query_degrades_cleanly_with_no_compute_surface(
    monkeypatch,
):
