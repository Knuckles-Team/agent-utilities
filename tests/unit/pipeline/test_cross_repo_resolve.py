...
async def test_resolve_cross_repo_import() -> None:
    ...
    # Raw import edge
    g.add_edge(
        "file_a", "placeholder_b", type="depends_on_raw", raw="agent_utilities.server"
    )
...
async def test_resolve_package_level_import() -> None:
    ...
    g.add_edge(
        "src", "placeholder", type="depends_on_raw", raw="agent_utilities.server"
    )
...
def test_build_package_map() -> None:
    ...
    g.add_node(
        "n3", type="file", name="readme.md", file_path="/workspace/README.md"
    )  # non-py should be skipped
