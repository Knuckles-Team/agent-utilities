...
class TestSchemaInitialization:
    ...
    def test_node_tables_count(self, engine):
        ...
        assert len(SCHEMA.nodes) >= 30, f"Expected 30+ node tables, got {len(SCHEMA.nodes)}"

    def test_relationship_tables_count(self, engine):
        """Verify we have all expected relationship tables."""
        assert len(SCHEMA.edges) >= 25, f"Expected 25+ rel tables, got {len(SCHEMA.edges)}"
...
class TestMemoryCRUD:
    ...
    def test_create_memory(self, engine):
        mem_id = engine.add_memory("Test content", name="Test", category="unit-test", tags=["tag1"])
        assert mem_id.startswith("mem:")
        # Verify in backend
        res = engine.query_cypher("MATCH (m:Memory {id: $id}) RETURN m.name as name", {"id": mem_id})
        assert len(res) > 0
...
class TestIngestion:
    ...
    def test_ingest_episode(self, engine):
        ...
        res = engine.query_cypher("MATCH (e:Episode {id: $id}) RETURN e.source as src", {"id": ep_id})
    ...
    def test_ingest_mcp_server(self, engine):
        ...
        # Verify server node
        res = engine.query_cypher("MATCH (s:Server {id: 'srv:filesystem'}) RETURN s.url as url")
        assert len(res) > 0
        assert res[0]["url"] == "http://localhost:3000"

        # Verify callable resources
        res = engine.query_cypher("MATCH (r:CallableResource) WHERE r.resource_type = 'MCP_TOOL' RETURN count(r) as cnt")
        assert res[0]["cnt"] >= 2

    def test_ingest_a2a_agent(self, engine):
        card = {"name": "CodeReviewer", "description": "Reviews code changes", "capabilities": ["review"]}
    ...
    def test_ingest_agent_skill(self, engine):
        frontmatter = {"name": "code-analyzer", "tags": ["analysis"], "description": "Analyzes code quality"}
        engine.ingest_agent_skill("/skills/code-analyzer/SKILL.md", frontmatter, "skill body...")
    ...
    def test_metadata_linkage(self, engine):
        ...
        engine.ingest_a2a_agent_card("http://test.io", {"name": "LinkedAgent", "description": "Test"})
        res = engine.query_cypher(
...
class TestMAGMARetrieval:
    ...
    def test_multi_view(self, engine):
        ...
        ctx = engine.retrieve_orthogonal_context("test", views=["semantic", "temporal", "entity"])
...
class TestAgentSpawning:
    ...
    def test_spawn_agent(self, engine):
        ...
        res = engine.query_cypher("MATCH (a:SpawnedAgent {id: $id}) RETURN a.system_prompt as prompt", {"id": agent_id})
...
class TestSelfImprovement:
    ...
    def test_generate_critique(self, engine):
        ...
        res = engine.query_cypher("MATCH (c:Critique {id: $id}) RETURN c.textual_gradient as grad", {"id": crit_id})
    ...
    def test_optimize_prompt(self, engine):
        ...
        # Create a critique
        crit_id = engine.generate_critique("ep:dummy", "Add specificity to instructions")
    ...
    def test_full_self_improvement_cycle(self, engine):
        ...
        res = engine.query_cypher("MATCH (p:SystemPrompt) WHERE p.source = 'REFINED' RETURN count(p) as cnt")
        assert res[0]["cnt"] > 0
...
class TestMaintenance:
    ...
    def test_importance_scoring(self, engine):
        ...
        engine.graph.add_edge(list(engine.graph.nodes)[0], list(engine.graph.nodes)[1], type="RELATED_TO")
    ...
    def test_temporal_decay(self, engine):
        ...
        res = engine.query_cypher("MATCH (n {id: $id}) RETURN n.importance_score as score", {"id": mem_id})
...
class TestGraphAlgorithms:
    ...
    def test_hybrid_search(self, engine):
        engine.graph.add_node("tool:python", type="tool", name="python_runner", description="Runs Python code")
