...
from agent_utilities.prompting.provider_adapter import (
    KGRuleBackend, ProviderPromptAdapter, ProviderRule, StaticRuleBackend,
)
...
class TestStaticBackend:
    ...
    def test_add_and_remove_rule(self):
        b = StaticRuleBackend()
        b.add_rule(ProviderRule(rule_id="t1", provider="openai", name="T1", priority=100))
...
class TestKGBackend:
    def test_add_retrieve_remove(self, mock_engine):
        ...
        b.add_rule(ProviderRule(rule_id="kg1", provider="openai", name="KG1", suffix="\nEnd."))
...
class TestPromptAdaptation:
    def test_openai_prefix(self, static_adapter):
        assert "specialized AI assistant" in static_adapter.adapt("Write code.", "openai")

    def test_anthropic_replacements(self, static_adapter):
        assert "Your task is to act as a" in static_adapter.adapt("You are a code expert.", "anthropic")
    ...
    def test_conditional_rule_applies(self):
        b = StaticRuleBackend()
        b.add_rule(ProviderRule(rule_id="c1", provider="openai", name="C1", priority=95,
                                suffix="\nCode only.", applicable_when={"task_type": "code"}))
        a = ProviderPromptAdapter(backend=b)
        assert "Code only." in a.adapt("Test", "openai", context={"task_type": "code"})
        assert "Code only." not in a.adapt("Test", "openai", context={"task_type": "chat"})

    def test_register_unregister(self, static_adapter):
        static_adapter.register_rule(ProviderRule(rule_id="r1", provider="mistral", name="R1"))
