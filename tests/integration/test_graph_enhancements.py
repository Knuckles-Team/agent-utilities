...
def test_agent_spawning(engine):
    ...
    agent_id = engine.spawn_specialized_agent(
        task_description="Build a rocket", tool_ids=["tool:hammer", "tool:wrench"]
    )
...
def test_self_improvement_loop(engine):
    ...
    engine.backend.execute(
        "CREATE (p:SystemPrompt {id: $id, content: 'Base', tags: ['agent-base']})",
        {"id": p_id},
    )
    ...
    res = engine.query_cypher(
        "MATCH (p:SystemPrompt) WHERE p.source = 'REFINED' RETURN count(p) as count"
    )
...
def test_maintenance_logic(engine):
    ...
    res = engine.query_cypher(
        "MATCH (n) WHERE n.importance_score IS NOT NULL RETURN count(n) as count"
    )
    assert res is not None
...
def test_temporal_decay(engine):
    ...
    engine.backend.execute(
        "MATCH (n) WHERE n.id = $id SET n.timestamp = '2020-01-01T00:00:00Z', n.importance_score = 1.0",
        {"id": mem_id},
    )
