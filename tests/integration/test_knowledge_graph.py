...
from agent_utilities.models.knowledge_graph import PipelineConfig, RegistryGraphMetadata, RegistryNodeType, RegistryEdgeType
...
def mock_graph():
    ...
    # Add an agent
    graph.add_node("TestBot", type=RegistryNodeType.AGENT, name="TestBot", description="A test bot", agent_type="prompt")
    # Add a tool
    graph.add_node("tool:search", type=RegistryNodeType.TOOL, name="search", description="Search tool", mcp_server="TestBot")
    # Link them
...
def test_intelligence_shortest_path(mock_graph):
    mock_graph.add_node("T2", type=RegistryNodeType.TOOL, name="T2", mcp_server="TestBot")
