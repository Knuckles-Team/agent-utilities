...
Synthesized from concept markers in the codebase into **1163 canonical concepts** across **9 pillars**.
...
| **AU-ECO** AU-ECO | AU-ECO.bus.agent-bus-awareness – AU-ECO.messaging.eco-2 | 132 | agent-to-agent messaging. Governed by the ActionPolicy, graph_bus MCP tool and REST twin for agent-to-agent messaging, AgentBus federated agent-to-agent communication bus over the KG, auto-register + online presence on any bus touch, bus register under the served auth profile, ────────────────────────, required engine-broker or Kafka log, the AgentBus is a NATIVE capability, not an opt-in persona |
| **AU-KG** AU-KG | AU-KG.audit.durable-error-detail – AU-KG.backend.cache-lives-as-248 | 495 | deterministic ownership disposition, routes to the engine, builds ON, PA-R1.7, the durable PostgreSQL graph tier executed via a bounded regex, the authority is the single shared, The authority has already acked; this must not wait on the, vector search competes for the same pool under load; |
