...
|---|---|---|---|---|
| 1 | Persistent dev workspace (stateful shell/files/tests/ports) | RLM sandbox = stateless snippet-against-namespace | **GAP** | OS-5.33 |
...
| 8 | Git issue/PR resolvers (6 platforms) + webhooks | MCP connectors only, no resolver | **GAP** | ECO-4.43 |
| 9 | Browser action tier (playwright/browsergym) | agent-browser skill, not in-loop | **GAP** | ECO-4.44 |
| 10 | React action/observation stream UI | agent-webui exists, no SWE view | **GAP** | OS-5.34 |
...
- **Self-evolution:** `adaptation/failure_analyzer.py` + `research/golden_loop.py` +
  AHE-3.14 governed merge. SWE-bench just needs to feed it (AHE-3.23).
...
Phase 1  Runtime           OS-5.33 + ORCH-1.46 + KG-2.64     ← long pole
Phase 2  SWE agent         KG-2.65 + ORCH-1.47               (KG-2.65 parallelizable)
Phase 3  Harness+loop      AHE-3.22 + AHE-3.23               ← highest value
Phase 4  Git resolver      ECO-4.43
Phase 5  Browser tier      ECO-4.44                          (optional)
Phase 6  webui stream      OS-5.34
```
