# Spec: Multi-Agent Scaling-Law Eval Harness (ORCH-1.49)

> Status: **proposed**. Co-satisfies SAFE-1.2 (implement as one harness — see
...
- [x] New CONCEPT:ORCH-1.49 justified: AU instruments collective **health** (per-run snapshots)
...
- [x] Wire-First confirmed: sweep → `ParallelEngine.run` per config → score fixed task suite →
  fit `capability ~ instances^α` → persist as KG calibration-style nodes (mirror KG-2.27),
...
- **AC1**: A `ScalingLawHarness` accepts a fixed task suite + a grid of `(N, density,
  ...
  token/compute per config (ECO-4.40 pricing / OS-5.23 metrics).
...
- **AC4**: Each fit is persisted as a KG node mirroring the KG-2.27 calibration-node shape
...
- `tests/unit/harness/test_orch_1_49_scaling_law_harness.py`
  (`@pytest.mark.concept(id="ORCH-1.49")`), ≤60s, no live engine/LLM — exercise the pure
...
- `pre-commit run --all-files` green; `docs/concepts.yaml` regenerated via
  ...
  authored for ORCH-1.49 (and the shared SAFE-1.2 note).
