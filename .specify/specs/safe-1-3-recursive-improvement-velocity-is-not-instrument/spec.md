# Spec: Recursive-Improvement Velocity Ledger (SAFE-1.3)
...
> productivity series on the existing ECO-4.41 observability router
...
- [x] Extension target identified: `_finalize_metrics` already emits the per-cycle ledger node;
      ...
      no one computes a rate. ECO-4.41 `usage_api.py` already exposes `/api/observability/*`.
- [x] New CONCEPT:SAFE-1.3 justified: this is the *measurement* axis (research-productivity curve +
      derivative), distinct from the loop itself (AHE-3.18), the regression gate (AHE-3.1), and cost
...
- [x] Wire-First confirmed: enrich the existing node write in `_finalize_metrics`; read it back over
      the existing ECO-4.41 router. No new daemon, no new loop, pure wiring over existing nodes.
...
- **AC1**: `_finalize_metrics` enriches the `evolution_cycle:` `orchestration_cycle` node with
  ...
  AHE-3.18 `GovernedAutoMerger` path), `eval_delta` (the AHE-3.1 regression-gate score delta, or
...
- **AC4**: `GET /api/observability/improvement-velocity` on the ECO-4.41 router queries the
...
- `tests/unit/knowledge_graph/test_safe_1_3_improvement_velocity.py`
  (`@pytest.mark.concept(id="SAFE-1.3")`), ≤60s, no live engine/LLM: a fake engine captures the
...
- `pre-commit run --all-files` green; `scripts/build_concepts_yaml.py` re-run so `docs/concepts.yaml`
  carries SAFE-1.3; `scripts/check_concepts.py` passes.
