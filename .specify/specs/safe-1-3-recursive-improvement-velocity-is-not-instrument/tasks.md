# Tasks: Recursive-Improvement Velocity Ledger (AU-OS.audit.recursive-improvement-velocity-tracker)

Wire-first: enrich the cycle node AU already writes, then read it back over the existing AU-ECO.mcp.usage-cost-observability-surface
...
1. **Enrich the existing ledger node.** In
   ...
   the report's publish/synthesize/auto-merge stages (the AU-AHE.harness.failure-evolution `GovernedAutoMerger` path),
...
2. **Add the pure velocity helper.** Add `improvement_velocity(cycles)` next to the loop (a new
   ...
   `CONCEPT:AU-OS.audit.recursive-improvement-velocity-tracker`, cite "From AGI to ASI" §7.4 in the docstring): compute
...
3. **Surface it on the existing observability router.** In `agent_utilities/gateway/usage_api.py`
   (AU-ECO.mcp.usage-cost-observability-surface, `/api/observability`) add `GET /improvement-velocity`: query the `orchestration_cycle`
...
4. **Test the live path.** Add `tests/unit/knowledge_graph/test_safe_1_3_improvement_velocity.py`
   (`@pytest.mark.concept(id="AU-OS.audit.recursive-improvement-velocity-tracker")`): a fake engine asserts `_finalize_metrics` writes the new
