...
2. **Add niche discovery** to `team_evolution.py`: `discover_specialization_niches(team_id)`
   ...
   competence threshold (no env flag). `CONCEPT:AU-ORCH.execution.swe-agent-system-prompt` tag in the docstring.
...
4. **Gate on the objective** using `social_system.py` (ORCH-1.32): mark the proposal
   ...
   continues down the existing AU-AHE.harness.failure-evolution regression-gated golden-loop path —
   `manifest_generators.py` (AU-ORCH.execution.autonomous-department-orchestration) consumes the role unchanged.
...
6. **Register + document:** add the `CONCEPT:AU-ORCH.execution.swe-agent-system-prompt` marker, run
