# Tasks: Compute-Optimal Test-Time Compute Governor (OS-5.33)
...
3. **Build `ComputeGovernor`** in a new `agent_utilities/harness/compute_governor.py`
   (CONCEPT:OS-5.33). Methods: `should_continue(samples, *, effort)` (AC1/AC2), `returns_curve()`
   ...
   ORCH-1.29/ORCH-1.2/ECO-4.40/OS-5.31 + paper §5.1 in the docstring.
...
5. **Wire the ledger sink (OS-5.31).** On stop, call
...
6. **Gate the live fan-out (Wire-First).** Make the existing consumer of `diverse_fan_out_width`
   ...
   (early-stop within the AHE-3.16 ceiling). This is the live invocation, not just an API.

7. **Tests.** Add `tests/unit/harness/test_os_5_33_compute_governor.py`
   (`@pytest.mark.concept(id="OS-5.33")`) covering AC1–AC4 + a `*_live_path` case driving the
