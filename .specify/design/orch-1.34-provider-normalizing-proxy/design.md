...
(AU-OS.governance.wasm-micro-agent-sandbox) and provenance-logged (KG)**, which a standalone chat proxy is not.
...
| AU-OS.governance.reactive-multi-axis-budget | Guardrails & Safety Boundaries | 0.46 | OS-5 |
| AU-OS.governance.wasm-micro-agent-sandbox | Telemetry & Observability | 0.34 | OS-5 |
| ECO-4.0 | Tool Interface & MCP Factory | 0.30 | AU-ECO.connector.plane-provisioning-auth |
...
### Extension Analysis
- **Primary Extension Point**: `core/model_factory.py` (ORCH-1.27) for the provider object; `security/guardrails.py` (AU-OS.governance.reactive-multi-axis-budget) for the SSRF gate.
...
- **Proposed ID**: `CONCEPT:AU-ORCH.adapter.byok-provider-proxy`
- **Augments Pillar**: ORCH (with an AU-OS.governance.reactive-multi-axis-budget guardrail hook)
...
C4Context
    ...
    System_Boundary(b1, "agent-utilities Core") {
        ...
        System(guard, "AU-OS.governance.reactive-multi-axis-budget Egress Guard", "validateBaseUrlResolved (SSRF)")
...
5. **OS**: egress guard (AU-OS.governance.reactive-multi-axis-budget) blocks internal IPs before fetch; token tracker (AU-OS.governance.wasm-micro-agent-sandbox) meters usage; credential resolver keeps keys out of logs/args.
