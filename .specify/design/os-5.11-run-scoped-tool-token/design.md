...
token to the **multi-tenant `ActorContext` (OS-5.1)** and the **ontological tool_guard (OS-5.10)**, so a
...
| OS-5.10 | Ontological Guardrail (tool_guard) | 0.61 | OS-5 |
| OS-5.3 | Guardrails & Safety Boundaries | 0.55 | OS-5 |
| ORCH-1.3 | Execution Safety & State | 0.41 | ORCH-1 |
| ECO-4.0 | Tool Interface & MCP Factory | 0.33 | ECO-4 |
...
### Extension Analysis
- **Primary Extension Point**: `security/brain_context.py` (`ActorContext`, OS-5.1) + `security/tool_guard.py` (OS-5.10).
...
C4Context
    ...
    System_Boundary(b1, "agent-utilities Core") {
        ...
        System(guard, "OS-5.10 tool_guard", "validate token + policy")
...
2. **KG**: token grant/revoke recorded for audit (OS-5.4).
...
5. **OS**: `tool_guard` (OS-5.10) validates signature/expiry/endpoint allowlist against the `ActorContext` (OS-5.1); revoked at run end/TTL.
