...
| **Object indexing** (batch + streaming, restrictions, staleness) | object-indexing-overview, object-backend | `ontology/indexing/` (`AU-KG.ontology.batch-incremental-sync-live`): `ObjectIndexFunnel` (batch full-rebuild + incremental/streaming deltas) gated by composable `DataRestriction`; `StalenessLedger` (content-hash + watermark drift) — drives the **same** `CapabilityIndex` the router ranks against. | **FULL** |
