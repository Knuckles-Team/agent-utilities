...
> **Concepts**: AU-039 (Confidence-Gated Router), AU-040 (Evolutionary Aggregation Engine)
>
...
1. **AU-039 — Confidence-Gated Router**: Adaptively selects cheaper or more
   expensive models based on runtime confidence signals from specialist consensus
   and historical performance.

2. **AU-040 — Evolutionary Aggregation Engine**: Groups specialist outputs,
...
## AU-039: Confidence-Gated Model Router
...
flowchart LR
    subgraph Signals
        WA[WorkspaceAttention<br/>AU-017]
        SM[SelfModel<br/>AU-016]
    end
...
### Composition with AU-033 (Homeostatic Downgrade)

The confidence-gated router composes with the existing budget-pressure
downgrade (AU-033):

1. **AU-033 runs first**: If the ResourceOptimizer detects budget pressure, it
   reduces the tier (e.g., `heavy → medium`).
2. **AU-039 runs second**: The confidence signal further adjusts within the
...
AU-039 optionally uses historical proficiency from AU-016 (Persistent Self-Model):
...
## AU-040: Evolutionary Aggregation Engine
...
```mermaid
graph LR
    AU016[AU-016<br/>Self-Model] -->|historical proficiency| AU039
    AU017[AU-017<br/>Workspace Attention] -->|confidence scores| AU039[AU-039<br/>Confidence Router]
    AU017 -->|proposals| AU040[AU-040<br/>Evolutionary Aggregation]
    AU033[AU-033<br/>Homeostatic Downgrade] -->|budget tier| AU039
    AU030[AU-030<br/>Cognitive Scheduler] -->|hosts| CM[Convergence<br/>Monitor]
    CM --> AU040
...
|:---|:---|
| [AU-016 Self-Model](overview.md) | Provides historical confidence priors |
| [AU-017 Workspace Attention](overview.md) | Provides runtime scoring and proposals |
| [AU-019 Resource Optimization](overview.md) | Budget-aware model selection |
| [AU-030 Cognitive Scheduler](overview.md) | Hosts convergence monitor |
| [AU-033 Homeostatic Downgrade](overview.md) | Composes with confidence routing |
