...
graph TD
    subgraph Existing ["Existing Utilities & Libraries"]
        ...
        KGCoord["mcp/kg_coordinator.py & LadybugDB<br/>(Transactional Graph Access)"]
...
2. **Knowledge Graph Transactions**:
   - All serialization, node storage, and edge creation utilize the unified `kg_coordinator.py` transactional pipeline via `LadybugDB` with full rollback support in case of structural failures.
...
These model definitions will bridge Cypher nodes dynamically to memory using the `LadybugDB` transaction proxy.
...
- **Specialist Routing & Department Orchestration (`ORCH-1.2` & `ORCH-1.27`)**: Allocate roles to specific specialized agents using `TeamConfigNode` under a dynamic legal department, leveraging the unified `legal_compliance_coordinator.json` structure.
...
- **SHACL Shape Validation (`shapes/legal.shapes.ttl`)**:
  Enforces structural completeness on legal nodes before they are written to the LadybugDB:
...
- **Interpretability and Safety Testing (`AHE-3.16`)**:
...
1. **PII Sanitizer Utility (`security/guardrails.py`)**:
   - We implement a robust `PiiSanitizer` class that intercepts all transaction payloads before they enter the pipeline.
...
#### B. Sandboxed Template Compilation (`OS-5.6`)
...
To successfully implement this, we lay out a **4-Phase Spec-Driven Development Plan** in alignment with `CONCEPT:ORCH-1.6` (DSTDD Pipeline).
...
- Run active reasoner compilation tests using the Rust FFI Oxigraph backend to verify transitive inheritance (e.g. ensuring `LegalTrust` correctly inherits from BFO concepts).
