...
* **No live-engine round-trip test in AU's own suite.** `evidence-graph` is part
  of epistemic-graph's mandatory main build, and AU's shared ephemeral-engine
  fixture (`tests/_test_engine.py`, `tiny_engine`/`engine_graph`) resolves the
  installed `epistemic-graph[full]` binary without an implicit Cargo fallback.
  There is still no `pytest.mark.engine` test in AU asserting
  `client.query.explain_evidence(...)` against that real running server for this
  seam. The EG-side Rust test is the closest proof today (it runs the actual
  engine decode/resolve code, just not over the wire). The natural follow-up is
  a normal AU engine test using the existing full fixture; no alternate engine
  build or test-only feature bundle is required.
