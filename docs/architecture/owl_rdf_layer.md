...
## Engine-native semantic web (CONCEPT:KG-2.204)
...
graph TB
    ...
    subgraph "OWL/RDF layer — engine-native (CONCEPT:KG-2.204)"
...
graph, CONCEPT:KG-2.204), falling back to per-node LPG iteration only when no engine
...
- `core/graph_compute.py::sparql()/owl_reason()/add_triples()/get_triples()` — the engine-native RDF surface (CONCEPT:KG-2.204): `client.rdf.sparql`/`owl_reason`/`add_triples`/`GetTriples`.
