...
- Every individual MCP server in the fleet (`agents/*`) ran its own
  ...
  validated" into `current_actor()` — so authorization had no verified caller
  context.
...
handlers, and anything they call into (like `identity_scoped_resources`),
  still had no verified actor. `ActorContextMiddleware` closes that gap
...
Graph administration is deliberately exact: only the effective capability
`kg:admin` grants it. A generic role or mapped capability named `admin` has no
special graph meaning. This keeps application administration separate from the
...
can see. Missing, unauthenticated, or tenantless identity is rejected before a
resource catalog is exposed. This is native and mandatory: no flag enables a
privileged fallback.
...
| --- | --- | --- |
| `AUTH_JWT_JWKS_URI` | unset | JWKS endpoint used to validate inbound Bearer JWTs (gateway `ActorIdentityMiddleware` and each server's own `JWTVerifier`). Required for every `streamable-http`/`sse` listener. |
| `AUTH_JWT_ISSUER` | unset | Expected `iss` claim; recommended alongside the JWKS URI. |
| `AUTH_JWT_AUDIENCE` | unset | Required expected `aud` claim and GraphSession audience. |
| `KG_POLICY_VERSION` | unset | Required immutable policy revision stamped into the GraphSession. |
...
Authentication is mandatory. The full authority contract
is documented in [Configuration Reference](configuration.md); this table lists
only what identity inheritance itself reads.
