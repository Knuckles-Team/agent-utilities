...
* `Method::ExplainProvenance` came closer (it already built a `KnowledgeSet` per
  request), but its former ad hoc row response dropped confidence, the bitemporal
  window, and policy labels before the response left the engine.
...
The current engine surface uses the authoritative Epistemic Operations Protocol:

1. **`EvidenceBundle` / `EvidenceClaim`** are generated from
   `evidence-bundle.schema.json` into strict Rust and Python projections. The query
   handler copies score, confidence, bitemporal ranges, source and evidence locus
   references, contradiction/proof references, and policy labels from the
   `KnowledgeRow`; unavailable evidence stays empty rather than being fabricated.
2. **`Method::ExplainProvenanceByIds { ids: Vec<String> }`** — a new, ID-seeded
   ...
   from `RowSet::from_ids(ids)` and resolves the same schema-bound `EvidenceBundle`
   that `ExplainProvenance` does. This is the primitive AU's facade calls.

The capability-policy ledger (`eg-capabilities`) classifies
`ExplainProvenanceByIds` identically to `ExplainProvenance`: read-only,
snapshot-isolated, and governed by `explain:read`.
