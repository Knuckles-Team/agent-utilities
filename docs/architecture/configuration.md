...
| `EPISTEMIC_GRAPH_STARTUP_TIMEOUT_SECS` | `300` | Readiness deadline for a locally autostarted unified engine. Large durable stores and one-time snapshot/`.mp`→redb migrations can delay listener bind for minutes; values are fail-closed and bounded to 5–1800 seconds. Configured remote engines do not use this timeout. |
| `GRAPH_SERVICE_ENDPOINTS` | unset | Engine coordinator/bootstrap contacts (comma/JSON list). 2+ entries = tenant-partitioned sharding via HRW over graph names; unset/1 = single-engine zero-infra default (CONCEPT:AU-KG.sharding.tenant-partitioned-sharding-hrw). This remains the only topology/autostart input: unset selects the local/self-contained path (share a running local engine, else autostart one); set selects the remote, connect-only path (never autostarted). For a placed group, authenticated `ClusterMembers` is the sole live endpoint/leader/certificate authority; static group maps and route endpoint hints are migration/audit inputs only. Three earlier, separately-configured switches for this same decision were retired outright — a config.json or environment that still carries any of them hard-fails boot (`core/config.py` `_RETIRED_CONFIGURATION_KEYS`). See `docs/recipes/unified-self-contained.md` |
| `GRAPH_CLUSTER_ID` | unset | Optional pinned `sha256:<digest>` cluster identity. A missing pin is still bound to the first authenticated discovery snapshot for the process and is never an endpoint source. |
| `GRAPH_CLUSTER_DISCOVERY_MAX_AGE_S` / `GRAPH_CLUSTER_DISCOVERY_CLOCK_SKEW_S` | `30` / `5` | Freshness and certificate-clock bounds for last-good `ClusterMembers` discovery; stale, wrong-context, or invalid snapshots fail closed. |
| `GRAPH_DRAIN_TIMEOUT_S` | `15` | Maximum GraphOS/MCP transport-drain wait on shutdown or pod termination; a timeout explicitly does not claim session continuity. |
...
| `MCP_ALWAYS_LOAD` | four core servers | Fleet servers mounted EAGERLY on a session's first contact, before any `find_tools` round trip. Ships `tunnel-manager-mcp`, `systems-manager-mcp`, `repository-manager-mcp`, `container-manager-mcp` — the cross-cutting operational capability an agent needs for almost any task. Mounting is fail-soft: a broken server logs loudly and degrades to lazy discovery, never blocking startup. `[]` restores fully-lazy (CONCEPT:AU-ECO.multiplexer.tool-gateway-catalog) |
| `MCP_ALWAYS_LOAD_TOOLS` | github/gitlab issues + PR/MR | INDIVIDUAL tools mounted eagerly, for servers too large to mount whole. Entries are `<server>:<tool>` (preferred — survives a prefix shift) or an already-prefixed name. Same fail-soft contract (CONCEPT:AU-ECO.multiplexer.tool-gateway-catalog) |
...
| `MCP_CATALOG_PROBE_TTL` | `300.0` | Freshness window (s) for a cached fleet-probe result. Past this age `list_catalog`/`find_tools` report the entry `stale: true` (computed from `age_s`, never a bare echo of the narrow in-flight `stale` flag) and `probe_catalog` re-targets it for a background re-probe — never blocking the caller, which still gets the honest last-known answer meanwhile (CONCEPT:AU-ECO.multiplexer.catalog-probe-ttl-staleness) |
...
- **remote** — any configured `GRAPH_SERVICE_ENDPOINTS` (single contact or a
  multi-shard topology): connect to it, **never** autostart a local stand-in
  (fail-loud if unreachable). This is the "I deployed the engine in Docker on
  another host" case (also the horizontal-scale-out shape). A remote/cluster
  engine is inherently persistent.
- **shared** — the local endpoint is already serving (a cheap connect probe
  ...
  ONE engine. This is also how the **unified self-contained** deployment behaves
  once its first process has started the engine — every later `graph-os` on the
  same host shares it instead of spawning a second one.
- **autostart** — nothing reachable on a local endpoint: spawn ONE **detached,
  ...
  Detached = it survives the spawner so OTHER entrypoints share it. Autostart is
  not an independent toggle — it is purely derived from topology
...
`engine_idle_shutdown_secs`) — set them in `config.json`, no env-sprawl. If the
installed engine binary doesn't advertise `--idle-shutdown-secs` yet (an
older/leaner wheel), the flag is omitted gracefully (the engine then runs
