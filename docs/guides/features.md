...
- **Agent Server (CONCEPT:AU-ECO.messaging.native-backend-abstraction)**: Built-in FastAPI server with `/mcp`, `/a2a`, and **`/docs`** endpoints, plus a separate Harness ACP stdio process for compatible editors.
...
- **Prompt Injection Scanner (CONCEPT:AU-OS.config.secrets-authentication)**: Pattern-based runtime threat detection with 25+ threat vectors covering reverse shells, data exfiltration, privilege escalation, encoded payloads, and prompt override attempts. Adapted from Goose's `scanner.rs`. Persists findings as `SecurityFindingNode` in the KG for OWL transitive risk propagation.
...
- **Guardrail Callback Engine (CONCEPT:AU-OS.config.secrets-authentication)**: Push-based input/output guardrail interception with block/redact/warn/log actions and regex/keyword pattern matching. Ported from MATE's `guardrail_callback.py`. OWL-promoted as `guardrail_trigger` nodes.
