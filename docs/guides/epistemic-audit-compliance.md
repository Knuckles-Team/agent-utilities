...
[Tiny](../recipes/tiny.md) recipe may not, depending on its XDG AgentConfig and
runtime injection.
...
purpose-named operation documented by `graph-query-and-explanation` over the same
...
- Skill: `graph-query-and-explanation` — the full four-layer epistemic-answer
...
- [Configuration Reference & Flag Audit](../architecture/configuration.md) —
  verified JWT/GraphSession identity plus mandatory fail-closed node
  ACLs) — the enforcement layer this guide's redaction sits on top of.
