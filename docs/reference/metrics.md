...
(CONCEPT:AU-OS.observability.no-op-without-metrics) — and incremented by the subsystem modules listed per table.
...
## Gateway HTTP (CONCEPT:AU-OS.observability.no-op-without-metrics)
...
|---|---|---|---|---|---|
| `agent_utilities_gateway_requests_total` | Counter | `route`, `method`, `status` | Gateway HTTP requests by route template, method, and status code | `observability/gateway_metrics.py` (middleware) | AU-OS.observability.no-op-without-metrics |
| `agent_utilities_gateway_request_duration_seconds` | Histogram | `route` | Request duration by route template | `observability/gateway_metrics.py` (middleware) | AU-OS.observability.no-op-without-metrics |
| `agent_utilities_gateway_in_flight_requests` | Gauge | — | Requests currently being handled by this process | `observability/gateway_metrics.py` (middleware) | AU-OS.observability.no-op-without-metrics |
| `agent_utilities_gateway_rate_limited_total` | Counter | `tenant` | Requests rejected (429) by the per-tenant token-bucket rate limiter | `gateway/rate_limit.py` | AU-OS.observability.no-op-without-metrics |

## Engine client & circuit breaker (CONCEPT:AU-OS.observability.no-op-without-metrics)

| Name | Type | Labels | Meaning | Emitted by (module) | Since (concept id) |
|---|---|---|---|---|---|
| `agent_utilities_gateway_engine_requests_total` | Counter | `op`, `outcome` | epistemic-graph engine client calls by operation and outcome (`ok` \| `connection_error` \| `error` \| `short_circuited`) | `knowledge_graph/core/engine_breaker.py` | AU-OS.observability.no-op-without-metrics |
| `agent_utilities_gateway_engine_breaker_state` | Gauge | `endpoint` | Engine circuit-breaker state per endpoint (0=closed, 1=half-open, 2=open) | `knowledge_graph/core/engine_breaker.py` | AU-OS.observability.no-op-without-metrics |

## Engine shard topology (CONCEPT:AU-KG.sharding.tenant-partitioned-sharding-hrw / AU-OS.scaling.shard-topology-visibility-per)
...
| Name | Type | Labels | Meaning | Emitted by (module) | Since (concept id) |
|---|---|---|---|---|---|
| `agent_utilities_engine_shard_up` | Gauge | `endpoint` | Per-shard engine reachability (1=reachable, 0=unreachable) | `knowledge_graph/core/shard_topology.py` | AU-KG.sharding.tenant-partitioned-sharding-hrw / AU-OS.scaling.shard-topology-visibility-per |
| `agent_utilities_engine_shard_requests_total` | Counter | `endpoint`, `outcome` | Engine client calls per shard endpoint and outcome (`ok` \| `connection_error` \| `error` \| `short_circuited`) | `knowledge_graph/core/engine_breaker.py` | AU-KG.sharding.tenant-partitioned-sharding-hrw / AU-OS.scaling.shard-topology-visibility-per |

## KG ingest queue backpressure (CONCEPT:AU-KG.ingest.decoupled-kg-ingest-consumer)
...
| Name | Type | Labels | Meaning | Emitted by (module) | Since (concept id) |
|---|---|---|---|---|---|
| `agent_utilities_mcp_child_calls_total` | Counter | `server`, `outcome` | Multiplexer tool calls per child and outcome (`ok` \| `error` \| `transport_error` \| `timeout` \| `busy` \| `unavailable` \| `short_circuited`) | `mcp/child_resilience.py` | AU-ECO.mcp.profile-differences-from-client |
...
## Agent communication bus (CONCEPT:AU-ECO.bus.operator-view-agentbus)

The agent-to-agent bus (`AgentBus`, AU-ECO.bus.agentbus-federated-agent-agent). Participant gauges are sampled on the
...
## ContextCompiler / Epistemic RAG (CONCEPT:AU-KG.retrieval.context-compiler / CONCEPT:AU-KG.retrieval.context-compiler-kv-seam)

The policy-aware context compiler (`knowledge_graph/retrieval/context_compiler.py`)
is the mandatory evidence-assembly boundary every Pydantic AI agent goes through
(`core/contextual_model.py::create_context_agent`/`wrap_model_with_context`) — this
is the delegated-run (`execute_agent`/`execute_workflow`) default context path
(W3.7), on by default, with the `MODEL_CONTEXT_COMPILER_ENABLED` deployment-level
escape hatch as the only opt-out (see `contextual_model._context_compiler_enabled`).
`ITEMS`/`TOKENS` are recorded on every `compile()` call; `KV_CACHE` only fires once a
Seam-6 KV backend is configured (`contextual_model.set_context_compiler_cache`, opt-in
— an external network-dependent performance layer, not part of the mandatory path).
`TTFT` is recorded from TWO call sites, disambiguated by the `path` label so the
interactive and batch latency populations are never averaged together:
`contextual_model._record_delegated_run_ttft` (every governed `execute_agent`/
`execute_workflow` model call) and `context_compiler_serving._record_ttft`
(`bundle_chat_completion`/`bundle_async_chat_completion`, the background
enrichment/extraction call sites).

| Name | Type | Labels | Meaning | Emitted by (module) | Since (concept id) |
|---|---|---|---|---|---|
| `agent_utilities_context_compiler_items_total` | Counter | `outcome` | Candidate items per `compile()` call by outcome (`selected` \| `dropped_policy` \| `dropped_redundant` \| `dropped_budget`) | `knowledge_graph/retrieval/context_compiler.py` | AU-KG.retrieval.context-compiler |
| `agent_utilities_context_compiler_tokens` | Histogram | `kind` | Per-`compile()` token counts (`in` = MMR-selected pool before budget fit, `selected` = tokens kept); selection efficiency = selected/in | `knowledge_graph/retrieval/context_compiler.py` | AU-KG.retrieval.context-compiler |
| `agent_utilities_context_compiler_kv_cache_requests_total` | Counter | `outcome` | Seam-6 bundle-level KV-cache lookups (`compile(kv_backend=...)`) by outcome (`hit` \| `miss`) — hit-rate = hit/(hit+miss) | `knowledge_graph/retrieval/context_compiler.py` | AU-KG.retrieval.context-compiler-kv-seam |
| `agent_utilities_context_compiler_ttft_seconds` | Histogram | `kv_cache_hit`, `path` | Wall-clock latency of a governed model call using a compiled bundle — a time-to-first-token PROXY, split by KV-cache-hit and by call-site population (`path="delegated_run"` \| `path="bundle_chat_completion"`) | `core/contextual_model.py`, `knowledge_graph/retrieval/context_compiler_serving.py` | AU-KG.retrieval.context-compiler-kv-seam |
...
25 Python-tier series documented above: 4 gateway HTTP, 2 engine client/breaker, 2
shard topology, 2 ingest queue, 4 MCP child, 3 dispatch, 4 agent bus, 4 ContextCompiler
— all registered in `observability/gateway_metrics.py` and verified against code in
