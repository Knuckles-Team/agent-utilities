...
from .base_utilities import GET_DEFAULT_SSL_VERIFY, to_integer, to_boolean, to_float, to_list, to_dict
...
class AgentConfig(BaseSettings):
    ...
    agent_description: str = Field(default=meta["description"], alias="AGENT_DESCRIPTION")
    ...
    graph_persistence_path: str = Field(default="agent_data/graph_state", alias="GRAPH_PERSISTENCE_PATH")
    enable_llm_validation: bool = Field(default=True, alias="ENABLE_LLM_VALIDATION")
    
                          
    custom_skills_directory: Optional[str] = Field(default=None, alias="CUSTOM_SKILLS_DIRECTORY")
    ...
    otel_exporter_otlp_public_key: Optional[str] = Field(default=None, alias="OTEL_EXPORTER_OTLP_PUBLIC_KEY")
    otel_exporter_otlp_secret_key: Optional[str] = Field(default=None, alias="OTEL_EXPORTER_OTLP_SECRET_KEY")
...
DEFAULT_LOGIT_BIAS = config.logit_bias if config.logit_bias is not None else to_dict(os.getenv("LOGIT_BIAS", None))
DEFAULT_STOP_SEQUENCES = config.stop_sequences if config.stop_sequences is not None else to_list(os.getenv("STOP_SEQUENCES", None))
DEFAULT_EXTRA_HEADERS = config.extra_headers if config.extra_headers is not None else to_dict(os.getenv("EXTRA_HEADERS", None))
DEFAULT_EXTRA_BODY = config.extra_body if config.extra_body is not None else to_dict(os.getenv("EXTRA_BODY", None))

DEFAULT_MIN_CONFIDENCE = config.min_confidence
DEFAULT_VALIDATION_MODE = config.validation_mode or to_boolean(os.getenv("VALIDATION_MODE", "False"))
...
DEFAULT_GRAPH_AGENT_MODEL = os.getenv("GRAPH_AGENT_MODEL", os.getenv("MODEL_ID", config.model_id))
