...
_AGENTS: tuple[tuple, ...] = (
    ...
    ("pi", "Pi", ("~/.pi/agent/sessions",), "PI_DIR", "**/*.jsonl", "generic", True),
    (
        "qwen",
        "Qwen Code",
        ("~/.qwen/projects",),
    ...
    ("kimi", "Kimi", ("~/.kimi/sessions",), "KIMI_DIR", "**/*.jsonl", "generic", True),
    (
        "kiro",
        "Kiro CLI",
        ("~/.kiro/sessions/cli", "~/.local/share/kiro-cli"),
        "KIRO_SESSIONS_DIR",
        "**/*.jsonl",
        "generic",
        True,
    ...
    (
        "claude-ai",
        "Claude.ai",
        ("~/.agent-utilities/imports/claude-ai",),
        "CLAUDE_AI_DIR",
        "**/*.json",
        "generic",
        True,
    ...
    (
        "chatgpt",
        "ChatGPT",
        ("~/.agent-utilities/imports/chatgpt",),
        "CHATGPT_DIR",
        "**/*.json",
        "generic",
        True,
    ),
    # Non-JSONL stores (SQLite/protobuf/encrypted): detected and registered with
    # the ``nonjsonl`` tag; a bespoke per-store reader is not yet wired.
    (
        "cursor",
        "Cursor",
        ("~/.cursor/projects",),
    ...
    ("zed", "Zed", ("~/.local/share/zed",), "ZED_DIR", "**/*.db", "nonjsonl", True),
    (
        "antigravity",
        "Antigravity",
    ...
    ("forge", "Forge", ("~/.forge",), "FORGE_DIR", "**/*.jsonl", "nonjsonl", True),
    (
        "warp-sqlite",
        "Warp (SQLite)",
    ...
    ("aider", "Aider", ("~/.aider",), "AIDER_DIR", "**/*.jsonl", "generic", True),
    (
        "cline",
        "Cline",
        ("~/.config/cline/tasks",),
...
NONJSONL_PENDING = frozenset(a[0] for a in _AGENTS if a[5] == "nonjsonl")
