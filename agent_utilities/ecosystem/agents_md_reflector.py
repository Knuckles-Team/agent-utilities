...
class AgentsMdProposal:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "proposal_id": self.proposal_id, "section": self.section,
...
class AgentsMdReflector:
    ...
    async def reflect(
        self, messages: list[dict[str, str]], *, session_id: str = "", source: str = "unknown",
        ...
        applied = [
            ...
            if self.auto_apply_confidence > 0 and p.confidence >= self.auto_apply_confidence
        ]
        for p in applied:
            try:
        ...
        logger.info("[ECO-4.2] Generated %d proposals (auto-applied: %d)", len(proposals), len(applied))
        return proposals
    ...
    def as_hook(self):
        ...
        async def _hook(input: HookInput) -> HookResult | None:
            ...
            if messages:
                await self.reflect(messages, session_id=str(getattr(input.ctx, "run_id", "")))
            return None
    ...
    async def _call_llm(self, transcript: str, current_md: str) -> str:
        try:
            ...
            model = create_model(provider=DEFAULT_LLM_PROVIDER, model_id=DEFAULT_KG_MODEL_ID)
    ...
    def _write_proposals(self, proposals: list[AgentsMdProposal]) -> None:
        ...
        if fp.exists():
            ...
            parts.append(f"# AGENTS.md Proposals — {time.strftime('%Y-%m-%d')}\n\n"
    ...
    def _persist_proposals(self, proposals: list[AgentsMdProposal], source: str) -> None:
        if not self.engine:
            return
        for p in proposals:
            try:
                self.engine.add_node(p.proposal_id, "agents_md_proposal", {
                    "name": f"Proposal: {p.section}", "description": p.reasoning,
                    "content": p.content, "confidence": p.confidence,
                    "source": source, "timestamp": p.timestamp,
                    "importance_score": p.confidence, "applied": False,
                })
            except Exception as e:
...
def apply_proposal(workspace_path: str | Path, proposal: AgentsMdProposal) -> None:
    ...
    if proposal.action == "add":
        if proposal.section in current:
            ...
            ns = current.find("\n## ", idx + 1)
            insert = f"\n{proposal.content}\n"
            current = current[:ns] + insert + current[ns:] if ns != -1 else current + insert
        else:
            current += f"\n\n{proposal.section}\n{proposal.content}\n"
...
def create_reflector_hook(engine=None, workspace_path=".", **kw):
    ...
    return AgentsMdReflector(engine=engine, workspace_path=workspace_path, **kw).as_hook()
        ...
        if hasattr(result, "all_messages"):
            for msg in result.all_messages():
                for part in getattr(msg, "parts", []):
                    ...
                    if text:
                        msgs.append({"role": getattr(msg, "role", "?"), "content": text[:2000]})
    except Exception:
        pass
    return msgs
