...
class EnhancedFileStatePersistence(BaseStatePersistence[StateT]):
    ...
    async def load_all(self, run_id: str) -> List[Union[NodeSnapshot[StateT], EndSnapshot[StateT]]]:
...
class PostgresStatePersistence(BaseStatePersistence[StateT]):
    ...
    async def snapshot_node_if_new(self, snapshot: NodeSnapshot[StateT]) -> None:
        ...
        async with pool.acquire() as conn:
            ...
            snapshot.run_id, 
            snapshot.timestamp, 
            getattr(snapshot, "snapshot_id", f"{snapshot.run_id}_{snapshot.timestamp.timestamp()}"),
            snapshot.node.node_id if hasattr(snapshot.node, "node_id") else str(snapshot.node),
            json.dumps(asdict(snapshot.node) if hasattr(snapshot.node, "__dataclass_fields__") else {}),
...
class RedisStatePersistence(BaseStatePersistence[StateT]):
    ...
    async def snapshot_node_if_new(self, snapshot: NodeSnapshot[StateT]) -> None:
        ...
        data = {
            ...
            "node_id": snapshot.node.node_id if hasattr(snapshot.node, "node_id") else str(snapshot.node),
            "state": TypeAdapter(type(snapshot.state)).dump_json(snapshot.state).decode(),
            "is_end": False
        }
