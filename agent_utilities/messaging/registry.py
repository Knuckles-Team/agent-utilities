"""Messaging Backend Registry with Entry-Point Discovery (CONCEPT:ECO-4.5).
...
class MessagingRegistry:
    ...
    def _discover(self) -> None:
        ...
        try:
            ...
            for ep in group_eps:
                ...
                logger.debug(
                    "[CONCEPT:ECO-4.5] Discovered messaging backend: %s → %s",
                    ep.name,
                    ep.value,
                )
        except Exception as e:
            logger.debug("[CONCEPT:ECO-4.5] Entry-point discovery failed: %s", e)

        if self._entry_points:
            logger.info(
                "[CONCEPT:ECO-4.5] Discovered %d messaging backends: %s",
                len(self._entry_points),
                ", ".join(sorted(self._entry_points)),
            )
        else:
            logger.debug(
                "[CONCEPT:ECO-4.5] No messaging backends discovered. "
    ...
    def create_backend(
        ...
        logger.info(
            "[CONCEPT:ECO-4.5] Created messaging backend: %s (%s)",
            backend_id,
        ...
        for backend_id in self.list_backends():
            ...
            if config.token or config.app_id:
                try:
                    ...
                    logger.warning("[CONCEPT:ECO-4.5] Skipping %s: %s", backend_id, e)
        return created
