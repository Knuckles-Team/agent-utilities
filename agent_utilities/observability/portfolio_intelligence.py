...
``certifications``/``attestations`` entry or an ``:attestsTo`` edge
...
nodes in the graph at all (substrate not ingested/federated), the required gate
fails closed.
...
def _unit_satisfied(
    ...
    (``:attestsTo``) targeting one of them. No evidence at all
    ...
    for _s, tgt, p in _out(engine, candidate_id):
        if _rel(p) == "attestsTo" and tgt in evidence_ids:
            return True
    return False
