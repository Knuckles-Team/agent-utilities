...
* **Non-blocking.** :meth:`SelfIngestSink.emit` only enqueues (``put_nowait``);
  ...
  blocks on network I/O, and a full queue drops records (bounded memory) rather
...
* **Zero new dependency.** Uses ``requests`` (already a core dependency),
  imported lazily. A transport callable can be injected for tests.
...
class SelfIngestSink:
    ...
    def emit(self, record: dict[str, Any]) -> None:
        """Enqueue one telemetry record. Non-blocking; drops when full."""
        if not self.enabled:
            return
        self.emitted += 1
        try:
    ...
    def flush(self) -> int:
        ...
        Used at shutdown and by tests (deterministic, no worker required).
        """
        sent = 0
        while True:
