...
def register_query_tools(mcp):
    ...
    def _run_graph_query(
        ...
        if _union_read and is_aggregation_cypher(cypher):
            engine = kg_server._get_engine()
            try:
                # Same defensive kwarg omission as the single-connection/fan-out
                # branches below: only pass `include_epistemic` when actually
                # requested, so a `query_cypher` implementation (real or test
                # double) that predates this parameter and doesn't accept it
                # keeps working.
