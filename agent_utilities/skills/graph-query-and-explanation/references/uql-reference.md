...
| **GIS / spatial** | ❌ **No** | — | `Op::SpatialScan{layer,bbox}` (source, `wire.rs:593`), `Op::Reproject{to_epsg,from_epsg}` (CRS transform, `wire.rs:603`), `Op::SpatialOp{kind}` (buffer/hull/simplify/…, `wire.rs:616`), and all 7 `Pred::Spatial*` filter predicates (`wire.rs:50-83`) are feature-`geo` wire types with **zero UQL grammar surface** — **§4.1 gap, the largest one**. The curated `graph_gis` MCP tool (documented in this same `graph-query-and-explanation` skill's action reference) is an unrelated **RPC-style** routing/tiling surface (`route`/`tile`/`nearest`/`geo_task`) — it is not RowSet-composable and does not go through `eg-plan` at all. |
...
| **KV-cache** | N/A — not a RowSet modality | — | `graph_kvcache` (`graph-runtime-and-governance` skill) is a content-addressed key→block store with `get`/`put`/`contains`/`stats` — it was never designed as a query modality and has no `Op`/RowSet integration; not a gap, just out of scope for UQL. |
