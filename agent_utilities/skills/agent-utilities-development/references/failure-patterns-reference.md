...
| **A hook's comment can outlive the gate's behaviour.** | Two `.pre-commit-config.yaml` comments still describe `scripts/openapi_coverage_baseline.json` and `scripts/wire_first_baseline.json` ratchets. **Neither file exists**; both baselines were retired 2026-08-28 under the workspace NO-RATCHETS rule and their `--update-*` flags now exit 2. Read the gate **script's** docstring, not the hook comment, before you reason about what a gate enforces. |
