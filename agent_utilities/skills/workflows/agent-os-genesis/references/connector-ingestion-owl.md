...
>
> Authoritative behavioral reference: the `graph-ingestion-and-integration` skill's
> [`references/ingest-connector-reference.md`](../../../graph-ingestion-and-integration/references/ingest-connector-reference.md)
> connector→OWL entity map. The code of record is `core/source_sync.py`
> (`_DELTA_HANDLERS`) + `connectors/mcp_tool.py` (`MCP_TOOL_PRESETS`). Keep all
> three in lockstep.
...
5. The row in `graph-ingestion-and-integration`'s
   [`references/ingest-connector-reference.md`](../../../graph-ingestion-and-integration/references/ingest-connector-reference.md)
   + this table.
