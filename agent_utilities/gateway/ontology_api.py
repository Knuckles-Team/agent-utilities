...
class OntologyLoadRequest(BaseModel):
    ...
    source: str = Field(
        ...,
        description="A UTF-8 Turtle file path or raw Turtle document.",
    )
    ...
    iri: str = Field(
        ...,
        description="Stable caller-owned ontology IRI.",
    )
    version: str = Field(..., description="Stable caller-owned ontology version.")
...
async def load_ontology(body: OntologyLoadRequest) -> OntologyEnvelope:
    """Attach an admin-owned ontology document through EG GraphSchema.
    ...
    (or a file path) to. EG validates the composed schema atomically; AU does
    not parse or retain the body.
    """
    if not body.source:
...
async def get_ontology_catalogue() -> OntologyEnvelope:
    """List generated GraphSchema metadata for admin-owned ontology sources."""
    return OntologyEnvelope(result=await _call("graph_ontology", action="list"))
