...
class SDDOrchestrator:
    ...
    def __init__(self, deps: AgentDeps, spec_generator: Callable[[str], Any] = None):
        self.deps = deps
    ...
    async def run_sdd(self, goal: str):
        ...
        if not self.spec_generator:
             logger.warning("No spec_generator provided to SDDOrchestrator. Skipping spec phase.")
        ...
        if self.deps.patterns:
            ...
            refactored = await self.deps.patterns.tdd_refactor_phase(spec, green_impl, red_tests)
        ...
        if self.deps.patterns:
            ...
            if "document" in goal.lower() or "walkthrough" in goal.lower():
                ...
                await self.deps.patterns.generate_walkthrough(str(self.deps.workspace_path))
