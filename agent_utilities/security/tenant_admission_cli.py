...
def run_tenant_admission(
    ...
    *,
    ...
    never touches a live engine or the ``client`` argument. Runs the SAME :func:`~agent_utilities.security.tenant_rbac_admission.provision_tenant_access`
    ...
    — the engine requires the signer to be the calling principal, so there is
    no separate provisioner credential to fetch — and, when ``client`` is not
    given, resolves a live engine via
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = run_tenant_admission(tenant_slug, principals, apply=args.apply)
