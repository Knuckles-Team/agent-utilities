...
* ``"<namespace>:*"`` / ``"<namespace>:admin"`` — entitles ALL resources in the
  namespace.
* ``"admin"`` (or a configured super-capability) — entitles all.
...
DEFAULT_SUPER_CAPS: frozenset[str] = frozenset({"admin"})
...
def grants_all_in_namespace(
    ...
    *,
    ...
    Triggered by a super-capability (``admin``) or a namespaced
...
def entitled_resources(
    ...
    *,
    ...
    Args:
        ...
        super_caps: capabilities that grant everything (default admin).
