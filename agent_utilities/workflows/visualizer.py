    ...
    def generate(manifest: ExecutionManifest, waves: list[list[AgentSpec]]) -> str:
        ...
        for wave_idx, wave_agents in enumerate(waves):
            ...
            for agent in wave_agents:
                ...
                shape = "round"
                if "synth" in node_id.lower() or "fusion" in node_id.lower() or "gate" in node_id.lower():
                    shape = "cylinder"
            ...
            builder.add_subgraph(
                ...
                nodes=[node.replace("_", "-") for node in wave_nodes],  # builder will sanitize again
                direction="LR"
            )
        ...
        builder.lines.append("  classDef success fill:#1b5e20,stroke:#2e7d32,color:#ffffff,stroke-width:2px;")
        builder.lines.append("  classDef active fill:#0d47a1,stroke:#1565c0,color:#ffffff,stroke-width:2px;")
        builder.lines.append("  classDef warning fill:#e65100,stroke:#f57c00,color:#ffffff,stroke-width:2px;")
