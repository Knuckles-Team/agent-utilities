...
# CONCEPT:ORCH-1.57 — curated per-task-class sampling profiles. Hand-tuned starting
...
class ModelRegistry(BaseModel):
    ...
    task_class_profiles: dict[str, SamplingProfile] = Field(
        default_factory=dict,
        description=(
            "CONCEPT:ORCH-1.57 — optional per-task-class sampling-profile overrides. "
    ...
    # ── CONCEPT:ORCH-1.57 sampling-profile selection ─────────────────────────────

    def pick_profile_for_task(self, task_class: str) -> SamplingProfile:
        """Return the sampling profile for a task-class (CONCEPT:ORCH-1.57).
    ...
    def pick_profile_for_role(self, role: ModelRole | str) -> SamplingProfile:
        """Return the sampling profile bound to a functional role (CONCEPT:ORCH-1.57).
...
def inference_owl_ttl(registry: ModelRegistry | None = None) -> str:
    """Project the registry's models + sampling profiles to OWL turtle (CONCEPT:KG-2.94).
    ...
    profiles across. Reuses the shared ``kg:`` namespace and the KG-2.95 edge names.
    """
...
# calls and is seen by the router/factory (ORCH-1.57) within the process.
...
def load_active_registry() -> ModelRegistry:
    ...
    CONCEPT:ORCH-1.57. Mirrors ``model_factory._resolve_role_model``'s load
