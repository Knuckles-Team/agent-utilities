...
class VariantPool:
    ...
    - **Fitness evaluation**: Aggregated from ``OutcomeEvaluationNode.reward``
      linked to canonical execution traces that used the variant
    ...
    def evaluate_fitness(self, variant_id: str) -> float:
        ...
        reachable via::
            ...
            (variant)<-[:USES_SKILL|USES_PROMPT]-(trace:RunTrace)
        ...
        if self.engine.backend:
            ...
            query = f"""
            MATCH (v {{id: $vid}})<-[:USES_SKILL|USES_PROMPT]-(r:RunTrace)
            -[:{TRACE_PRODUCED_OUTCOME_EDGE}]->(eval:OutcomeEvaluation)
