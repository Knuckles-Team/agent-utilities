...
async def _run(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    ...
    pipeline = _create_pipeline(args, config)

    if args.bootstrap_workspace:
        await _bootstrap_workspace(args.bootstrap_workspace)
        return

    if not await _run_command(args, pipeline, config):
        parser.print_help()
...
def _create_pipeline(
    ...
    if args.stage_to_queue:
        ...
        return IntelligencePipeline(config, graph_name=f"stage_{args.stage_to_queue}")
    return IntelligencePipeline(config)


async def _bootstrap_workspace(workspace_spec: str) -> None:
    from agent_utilities.core.workspace_config import clone_missing_projects

    print("Bootstrapping workspace...")
    if workspace_spec == "DEFAULT":
        project_paths = clone_missing_projects()
    else:
        project_paths = clone_missing_projects(yml_path=workspace_spec)
...
async def _run_command(
    ...
    commands = (
        ("maintain", _maintain_pipeline),
        ("status", _show_status),
        ("search", _search_graph),
        ("impact", _show_impact),
    ...
    for argument, command in commands:
        if getattr(args, argument, None):
            await command(args, pipeline, config)
            return True
    return False


async def _maintain_pipeline(
    args: argparse.Namespace,
    pipeline: IntelligencePipeline,
...
def _stage_pipeline(
    ...
    from agent_utilities.knowledge_graph.core.queue_backend import create_task_queue
    ...
    _cleanup_scratch_tenant(pipeline)


def _cleanup_scratch_tenant(pipeline: IntelligencePipeline) -> None:
    # Drop the ephemeral scratch tenant — its payload now lives in the staging
    # queue and will be merged into "__commons__" by the graph-writer.
    try:
        ...
        logging.getLogger(__name__).debug(f"Scratch tenant cleanup skipped: {e}")
...
async def _search_graph(
    ...
    await pipeline.run()
    engine = IntelligenceGraphEngine.get_or_create(graph=pipeline.graph)
    results = engine.search_hybrid(args.search)
    if results:
        for result in results:
            print(
                f"[{result.get('type', 'node').upper()}] [ID: {result.get('id')}] {result.get('name')}: {result.get('description', '')[:100]}..."
            )
    else:
        print(f"No results found for '{args.search}'.")


async def _show_impact(
    args: argparse.Namespace,
    pipeline: IntelligencePipeline,
    _config: PipelineConfig,
) -> None:
...
async def _search_memories(
    ...
    memories = [memory for memory in memories if memory.get("type") == "memory"]
    if memories:
        for memory in memories:
            print(
                f"[{memory.get('timestamp', 'N/A')}] {memory.get('name')}: {memory.get('description', '')[:100]}..."
            )
    else:
