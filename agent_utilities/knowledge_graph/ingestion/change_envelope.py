...
* **operational** — ``provenance`` (free-form dict: manifest hash, connector
  version, generated_by), ``confidence``, ``checkpoint`` (the engine-owned typed
  source cursor committed with the material), ``trace_context`` (W3C traceparent,
  mirroring :class:`~knowledge_graph.core.session.GraphSession`).

The type includes deterministic connector-record and entity projections used by
the mapping and native DTO layers. The sole ingest consumer is
:func:`~..ingestion.envelope_ingest.ingest_envelope`: it renders the DTO into
Epistemic Graph's native ``ApplyChangeEnvelope`` so object/blob/features,
evidence, policy, lineage, content version, cursor, and outbox share one redb
transaction and (when clustered) one Raft entry.
"""
...
class ChangeEnvelope:
    ...
    Construction defaults support deterministic connector translators; native
    apply validation still requires the identity, access, provenance, and cursor
    ...
    Attributes:
        ...
        source_version: The upstream object's own version/updated marker (the
            ...
            from the engine-owned source cursor.
        ...
        checkpoint: The typed source cursor value to commit atomically once this
            envelope is durably processed.
    ...
    def from_connector_record(
        ...
        **overrides: Any,
        ...
        """Translate a governed connector record into the canonical envelope.

        Connectors emit ``{"id": ..., "type": ..., **props}`` records; this
        translator binds the record to its connector, source version, tenant,
        ACL, and provenance before the native apply operation.
        ...
        Args:
            ...
            session: The verified ``GraphSession`` (or an internal object exposing
                ``.tenant``/``.trace_context``) supplying tenant and trace context
        ...
        if session is None and (not tenant or "trace_context" not in overrides):
            ...
            except Exception:  # noqa: BLE001 — explicit tenant remains supported
                session = None
    ...
    def snapshot_complete(
        ...
        **overrides: Any,
        ...
        snapshot asserts. The native consumer resolves stale rows and commits
        the marker, tombstones, policy, lineage, cursor, and outbox together.
        ``fetch_ok=False`` (or an unapproved authoritatively-empty source)
        retains the existing live set, so a failed/skipped fetch cannot become
        an accidental wipe.
    ...
    def to_entity_dict(self) -> dict[str, Any]:
        """Render the typed payload as the native entity projection.
