...
class SchemaMapping(BaseModel):
    ...
    ontology_class: str | None = (
        None  # ArchiMate/base crosswalk parent (consumed by manifest_compiler + ops_causal_crosswalk)
    )
...
class ProvenanceSpec(BaseModel):
    ...
    """The git commit SHA of this repository at generation time
    (:func:`ontology_integrity.repository_commit_sha`). Answers "which commit
    produced this manifest" — nothing more. Replaces the old signed
    ``dependency_lock_digest`` pin (GOC-84/GOC-16, removed): that pin made every
    dependency addition a release-Job-gated operation (any ``uv.lock`` change
    invalidated the pin in all bundled manifests) for no benefit in an internal
    deployment. ``None`` only for a hand-authored manifest generated outside a
    git checkout.
    """
