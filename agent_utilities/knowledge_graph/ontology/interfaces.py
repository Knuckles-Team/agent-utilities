...
def register_builtin_interfaces(registry: InterfaceRegistry) -> None:
    ...
    # CONCEPT:KG-2.94 — InferenceProfile + SamplingConfigurable ontology interfaces and a first-class Model object type projecting configured models and their tuned sampling profiles to OWL.
    # Task-aware inference profiles. An InferenceProfile is the
    # shape of a SamplingProfile (ORCH-1.57): the sampling knobs as shared
    # properties. The bounds are enforced by the matching value types (KG-2.93,
    ...
    registry.register(
        Interface(
            ...
            description=(
                ...
                "for a task class. Bounds enforced by the KG-2.93 value types."
            ),
            properties=[
                InterfaceProperty(
                    name="task_class",
    ...
    # CONCEPT:KG-2.94 — inference-profile shapes ship with live implementers.
    registry.declare_type_shape(
