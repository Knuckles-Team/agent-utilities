...
VALUE_TYPES: dict[str, ValueType] = {
    ...
    # CONCEPT:KG-2.93 — LLM inference sampling knobs temperature top_p top_k min_p repetition_penalty max_tokens and penalties as SHACL-bounded ontology value types plus the two-surface sampling-profile tool.
    # A SamplingProfile (the InferenceProfile interface, KG-2.94) is SHACL-checked at the
...
# CONCEPT:KG-2.93 — the coupling from a SamplingProfile's knobs to the value types
...
def sampling_profile_violations(profile: dict[str, Any]) -> list[str]:
    """Return value-type violations for a sampling-profile dict (CONCEPT:KG-2.93).
