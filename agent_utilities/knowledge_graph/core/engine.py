...
class IntelligenceGraphEngine(
    ...
    def link_nodes(
        ...
        *,
        ...
        Write ordering follows the configured authority contract: the backend
        is written first (it is always the source of truth), and the compute
        scratchpad is updated afterward only when it is a distinct view from
        that backend — an EpistemicGraphBackend write already passed through
        the same compute client, so no redundant second write happens there.
        ...
        with use_actor(session.actor):
            ...
            # Tier 2: An EpistemicGraphBackend write already passed through this
            # exact compute client — only other durable stores (a distinct,
            # non-authoritative compute scratchpad) need the second write.
    ...
    def add_node(
        ...
        *,
        ...
        with use_actor(session.actor):
            ...
            # Tier 2: An EpistemicGraphBackend write already passed through this
            # exact compute client — only a distinct, non-authoritative compute
            # scratchpad needs the second write.
            if ephemeral or not self._compute_is_authority:
