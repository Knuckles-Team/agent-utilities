...
_SYSTEM_PROMPT = (
    ...
    "`nodes`/`edges` columns (label questions filter on `node_type`, not `label`; "
...
def build_schema_context(engine: Any, *, max_labels: int = 60) -> dict[str, Any]:
    ...
    Label probing reads canonical ``n.node_type`` directly rather than the
    ...
    try:
        rows = engine.query_cypher(
            "MATCH (n) RETURN n.node_type AS nt "
