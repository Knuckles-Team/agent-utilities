...
class FinanceEngineMixin(_Base):
    ...
    def register_trading_startegy(
        self, name: str, asset_class: str, timeframe: str
    ) -> str:
        """Register a new trading startegy into the KG."""
        startegy_id = f"start:{uuid.uuid4().hex[:8]}"
        ...
        if self.backend:
            ...
            self._upsert_node("TradingStrategy", startegy_id, data)
        return startegy_id

    def generate_execution_signal(
        self, startegy_id: str, direction: str, strength: float, target_price: float
    ) -> str:
        """Record an execution signal generated by a trading startegy."""
        ...
        if self.backend:
            ...
            self.backend.execute(
                ...
                {"sid": startegy_id, "sigid": signal_id},
            )
        return signal_id
