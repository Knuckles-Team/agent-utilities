...
Engine-only (CONCEPT:AU-KG.memory.time-series-lives-one): time-series points live on the **one
...
def get_timeseries_backend(backend_type: str = "engine", **kwargs) -> TimeSeriesBackend:
    ...
    CONCEPT:AU-KG.memory.time-series-lives-one — there is one backend: the epistemic-graph engine tsdb. It
    ...
    if backend_type != "engine":
        raise ValueError(
            ...
            "(CONCEPT:AU-KG.memory.time-series-lives-one) — time-series lives on the one engine authority."
        )
