...
"""CONCEPT:AU-AHE.evaluation.backtest-harness — Serialized Memory Cache for Heavy Thinking.
...
- **KG persistence**: Serializes all trajectories as ``TrajectoryNode``
  instances for cross-session reuse (free value-add from CONCEPT:AU-KG.query.object-graph-mapper)

Integrates with:
    - CONCEPT:AU-AHE.evaluation.backtest-harness (Heavy Thinking): Core data structure for the pipeline
    - CONCEPT:AU-KG.query.object-graph-mapper (OGM): KG persistence via ``KGMapper``
    - CONCEPT:AU-KG.compute.cross-pillar-synergy (Hypergraphs): EncPI metadata on trajectory interactions
...
class TrajectoryPruner:
    ...
    CONCEPT:AU-AHE.evaluation.backtest-harness — Prevents context overflow in the deliberation
...
class TrajectoryShuffler:
    ...
    CONCEPT:AU-AHE.evaluation.backtest-harness — The deliberation model may exhibit position bias
