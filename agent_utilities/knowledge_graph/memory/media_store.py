...
--------------------------------------------------------------------
The identity model separates *what the bytes are* from *how, when, and under which
authority they occurred*. Content identity and occurrence provenance can therefore
never collapse onto the same node:
...
class MediaStore:
    ...
    def store_code_symbol_evidence(
        ...
        **store_media_kwargs: Any,
        ...
        extraction reached via ``client.graph``/``graph-query-and-explanation``) already resolves
