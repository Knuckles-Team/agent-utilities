...
Integrates with:
    - CONCEPT:KG-2.10 (ContextCompactor): Uses display budgets in prompts
    - CONCEPT:KG-2.16 (SemanticSubsumption): Classifies display strategies
    - CONCEPT:KG-2.15 (AnalogyEngine): Finds similar model displays
...
class ModelDisplayOptimizer:
        ...
        for feat in feature_data:
            ...
            if slope is not None:
                ...
                for i, (k, c) in enumerate(
                    zip(knot_values[: self._budget.max_knots], coefficients)
                ):
    ...
    def _symbolic_equation(
        ...
        lines = [l.strip() for l in model_str.strip().split("\n") if l.strip()]
        if not lines:
            return model_str
        equation = lines[0]
