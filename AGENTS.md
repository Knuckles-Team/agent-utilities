...
1. **Pre-commit green** — `python3 scripts/safe_precommit_all_files.py` (the safe
   `--all-files` wrapper, D-OB-12 — see *Quality Bar*); resolve every issue per the
...
_Auto-generated from `docs/concepts.yaml` (single source of truth). 1059 concepts across 9 pillars._
...
| **AU-KG** | 451 | audit, backend, compute, coordination, domains, enrichment, epistemic, etl, evolution, identity, ingest, maintenance, memory, mining, ontology, query, research, retrieval, sharding, storage, temporal, txn |
